...
class TransactionUtils {
  ...
  areWellFormedJupiterTransactionIds(transactionIds) {
    ...
    const someAreNotWellFormed = transactionIds.some(
      (transactionId) => !this.isWellFormedJupiterTransactionId(transactionId)
    )
  ...
  isValidEncryptedMessageTransaction(messageTransaction) {
    ...
    if (
      !(
        messageTransaction.hasOwnProperty('attachment') &&
  ...
  isValidEncryptedMessageTransactionResponse(transactionResponse) {
    ...
    if (
      !(
        transactionResponse.hasOwnProperty('transactionJSON') &&
  ...
  filterMessageTransactionsByCallback(transactions, callback) {
    logger.verbose(
      `#### filterMessageTransactionsByCallback(transactions, callback): transactions.length = ${transactions.length} `
    )
    if (transactions.length === 0) {
  ...
  filterMessageTransactionsBySenderOrRecipient(transactions, recipientOrSenderAddress) {
    ...
    return this.filterMessageTransactionsByCallback(
      transactions,
      (transaction) =>
        transaction.recipientRS === recipientOrSenderAddress || transaction.senderRS === recipientOrSenderAddress
    )
  }
  ...
  filterMessageTransactionsByRecipient(transactions, recipientAddress) {
    ...
    return this.filterMessageTransactionsByCallback(
      transactions,
      (transaction) => transaction.recipientRS === recipientAddress
    )
  }
