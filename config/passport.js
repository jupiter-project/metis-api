...
const metisRegistration = async (address, requestBody) => {
  ...
  const userAccountProperties = await instantiateGravityAccountProperties(
    signUpUserInformation.passphrase,
    signUpUserInformation.encryption_password
  )

  const registration = accountRegistration.register3(userAccountProperties, signUpUserInformation.alias)
