...
module.exports = {
  validate_model: (attributeName, attributeValue, requirements) => {
    ...
    if (
      requirements.dataType !== 'Integer' &&
      requirements.dataType !== 'String' &&
      requirements.dataType !== 'Boolean' &&
      requirements.dataType !== 'Email' &&
      requirements.dataType !== 'Cryptocurrency' &&
    ...
    if (requirements.dataType === 'String' || requirements.dataType === 'Text') {
      ...
      if (
        ...
        if (requirements.maxLength <= requirements.minLength) {
          errors = true
          errorMessages.push(
            `${attributeName}'s maximum character length cannot be less or equal to minimum character length.`
          )
        }
      }
    }
