...
class Invite extends Model {
  constructor(data = { id: null }) {
    ...
    super({
      ...
      table: 'channels',
      belongsTo: 'user',
      model_params: ['id', 'recipient', 'sender', 'channel']
    })
  ...
  async get() {
    ...
    return instantiateGravityAccountProperties(this.user.passphrase, this.user.encryptionPassword).then(
      (memberAccountProperties) => {
        // @TODO we need to use tags here!!
        return jupiterTransactionsService
          .getAllConfirmedAndUnconfirmedBlockChainTransactions(this.user.account)
          .then((transactionList) => {
            return jupiterTransactionsService.getAllMessagesFromBlockChainAndIncludeTransactionInformation(
              memberAccountProperties,
              transactionList,
  ...
  async send() {
    ...
    const inviteRecord = { ...messageData }
