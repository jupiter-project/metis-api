...
class UploadJob {
        ...
        this.jobQueue.process(this.jobName, WORKERS, async (job,done) => {
            ...
            try {
                ...
                if(fileCategory === FILE_CATEGORY_TYPES.PUBLIC_PROFILE){
                    ...
                    if(!job.data.hasOwnProperty('attachToJupiterAddress')) return done(new mError.MetisError('attachToJupiterAddress is missing'));
                    const attachToJupiterAddress = job.data.attachToJupiterAddress;
                    if(!gu.isWellFormedJupiterAddress(job.data.attachToJupiterAddress)) return done(new mError.MetisErrorBadJupiterAddress('attachToJupiterAddress', job.data.attachToJupiterAddress));
    ...
    create(ownerAccountProperties, fileName, fileEncoding, fileMimeType, fileUuid, fileCategory, params={}){
        ...
        if(fileCategory === FILE_CATEGORY_TYPES.PUBLIC_PROFILE){
            ...
            if(!params.hasOwnProperty('attachToJupiterAddress')) throw new mError.MetisError('attachToJupiterAddress is missing');
