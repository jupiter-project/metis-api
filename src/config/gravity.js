...
class Gravity {
      ...
      self
        ...
        .then((response) => {
          if (returnType === 'console') {
            logger.info(
              `Database tables associated with your app ${response.app.appData.name} (${response.app.address})`
            )
    ...
    return new Promise((resolve, reject) => {
      ...
      this.getRecords(
        ...
        .then((recordsContainer) => {
          ...
          const currentTables = [currentUsersTable, currentChannelsTable, currentInvitesTable, currentStorageTable]
  ...
  getMessages(address, passphrase) {
    ...
    return new Promise((resolve, reject) => {
      ...
      axios
        .get(`${self.jupiter_data.server}/nxt?requestType=getBlockchainTransactions&account=${address}&type=1`)
        .then((response) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      eventEmitter.on('database_retrieved', () => {
        ...
        for (
          ...
          transactionsIndex < Object.keys(blockChainTransactions).length;
          transactionsIndex += 1
        ) {
          ...
          if (
            blockChainTransactions[transactionsIndex].attachment.encryptedMessage &&
            blockChainTransactions[transactionsIndex].attachment.encryptedMessage.data
          ) {
            // console.log(1)
            // logger.debug(`transactionSender= ${transactionSender} `);
            ...
            if (blockChainTransactions[transactionsIndex].senderRS === transactionSender) {
              ...
              // console.log(9)
              logger.warn(
                `${transactionsIndex} : Wrong SenderRs: ${blockChainTransactions[transactionsIndex].senderRS}. Needs to be by the transactionSender ${transactionSender}`
              )
            }
            // console.log(10)
          } else {
      ...
      jupiterTransactionsService
        .getAllConfirmedAndUnconfirmedBlockChainTransactions(ownerAddress)
        .then((transactions) => {
  ...
  getAppRecords(appAddress, appPassphrase) {
    ...
    return new Promise((resolve, reject) => {
      ...
      axios
        .get(`${self.jupiter_data.server}/nxt?requestType=getBlockchainTransactions&account=${appAddress}&type=1`)
        .then((response) => {
          database = response.data.transactions
          eventEmitter.emit('database_retrieved')
        })
  ...
  getAllRecords(table, scope = { size: 'all', show_pending: null }) {
    ...
    return new Promise((resolve, reject) => {
      ...
      eventEmitter.on('table_access_retrieved', () => {
        axios
          .get(
            `${self.jupiter_data.server}/nxt?requestType=getBlockchainTransactions&account=${recordTable.address}&type=1`
          )
          .then((response) => {
      ...
      logger.debug('-- ---- -- --- -- --- $$$$$ -- ---- -- --- -- --- 4')
      self
        .loadUserAndAppData(scope.containedDatabase)
        .then((res) => {
  ...
  getUser(account, passphrase, containedDatabase = null) {
    logger.verbose('##########################################################################################')
    logger.verbose(
      `## getUser(account=${JSON.stringify(account)}, passphrase, containedDatabase=${!!containedDatabase})`
    )
    logger.verbose('##')
    ...
    return new Promise((resolve, reject) => {
      if (account === process.env.APP_ACCOUNT_ADDRESS) {
        ...
        logger.verbose(
          ' With the Application Users Table we can find the user credentials inorder to authenticate the user'
        )
        ...
        this.getRecords(
          ...
          // ------------------------------------------------------------------------------------------------------------------------------
          logger.verbose(
            'So now we have the Application Users Table! Next we need to find the user in ApplicationUsersTable.'
          )
          ...
          this.getRecords(
            ...
            .then((applicationAccountUsersTableRecordsResponse) => {
              logger.debug('---------------------------------------------------------------------------------------')
              logger.verbose(
                'getUser().getRecords(from application).then().getRecords(applicationUsersTable).THEN(applicationUsersTableRecordsResponse)'
              )
              logger.debug('---------------------------------------------------------------------------------------')
              ...
              if (userRecord) {
                logger.sensitive(
                  `-- retreived user password = ${userRecord.encryption_password}, containedDatabasePassword = ${containedDatabase.encryptionPassword}`
                )
                logger.sensitive(
                  `-- retreived user password = ${userRecord.encryption_password}, containedDatabasePassword = ${containedDatabase.encryptionPassword}`
                )
              ...
              // ------------------------------------------------------------------------------------------------------------------------------
              self
                .retrieveAccountTables(containedDatabase, applicationUsersTableAddress)
                .then((userAccountTablesResponse) => {
                  logger.debug(
                    '---------------------------------------------------------------------------------------'
                  )
                  logger.debug(
                    `-- getUser().retrieveUserFromPassphrase(containedDatabase = ${!!containedDatabase}).THEN(userFromPassphraseResponse)`
                  )
                  logger.debug(
                    '---------------------------------------------------------------------------------------'
                  )
  ...
  retrieveAccountTables(accessData) {
    ...
    return new Promise((resolve, reject) => {
      ...
      let userRecord
      logger.debug(
        `retrieveUserFromPassphrase(accessData=${!!accessData}).loadUserAndAppData(accessData = ${!!accessData})`
      )
      this.loadUserAndAppData(accessData)
        .then((userAndAppData) => {
          ...
          logger.debug('---------------------------------------------------------------------------------------')
          logger.debug(
            `-- retrieveUserFromPassphrase(accessData=${!!accessData}).loadUserAndAppData(accessData=${!!accessData}).THEN(userAndAppData=${!!userAndAppData})`
          )
          logger.debug('---------------------------------------------------------------------------------------')
  ...
  retrieveUserFromApp(userAccount, passphrase) {
    ...
    return new Promise((resolve, reject) => {
      ...
      eventEmitter.on('set_responseData', () => {
        ...
        logger.sensitive(
          `Total appAcctUserTableDecryptedMessagesSentToUser: ${appAcctUserTableDecryptedMessagesSentToUser.length}`
        )
        logger.sensitive(
          `appAcctUserTableDecryptedMessagesSentToUser= ${JSON.stringify(appAcctUserTableDecryptedMessagesSentToUser)}`
        )
        if (
          appAcctUserTableDecryptedMessagesSentToUser[0] === undefined ||
      ...
      eventEmitter.on('records_retrieved', () => {
        ...
        logger.debug(
          `Total  appAcctUserTableTransactionsSentToUserAcct to process: ${appAcctUserTableTransactionsSentToUserAcct.length}`
        )
      ...
      eventEmitter.on('table_access_retrieved', () => {
        ...
        logger.sensitive(
          `getting transactions from applicationAccountUserTable: ${applicationAccountUserTable.address}`
        )
        const includeExpiredPrunable = '&includeExpiredPrunable=true'
        axios
          .get(
            `${self.jupiter_data.server}/nxt?requestType=getBlockchainTransactions&account=${applicationAccountUserTable.address}&type=1${includeExpiredPrunable}`
          )
          .then((response) => {
            ...
            logger.sensitive(
              `applicationAccountUserTableTransactions Count: ${applicationAccountUserTableTransactions.length}`
            )
  ...
  getBalance(address, accountId, jupServ) {
    ...
    return new Promise((resolve, reject) => {
        ...
        axios
          ...
          .then((response) => {
            if (response.data.errorDescription) {
              ...
              logger.info(
                `Balance: ${parseFloat(response.data.balanceNQT) / 10 ** self.jupiter_data.moneyDecimals} JUP.`
              )
      ...
      if (!accountId) {
        axios
          .get(`${server}/nxt?requestType=getAccountId&secretPhrase=${addressOwner}`)
          .then((response) => {
  ...
  sendMoneyAndWait(recipient, transferAmount, sender, secondsToWait = 50) {
    logger.verbose('#####################################################################################')
    logger.verbose(
      ` sendMoneyAndWait(recipient=${recipient}, tansferMoney=${transferAmount}, sender=${sender}, seccondsToWait= ${secondsToWait})`
    )
    logger.verbose('#####################################################################################')
  ...
  async getAlias(aliasName) {
    ...
    const aliasCheckup = await this.jupiterRequest('get', { aliasName, requestType: 'getAlias' })
  ...
  createNewJupiterAccount(passphrase) {
    return new Promise((resolve, reject) => {
      axios
        .get(`${this.jupiter_data.server}/nxt?requestType=getAccountId&secretPhrase=${passphrase}`)
        .then((response) => {
  ...
  getAccountInformation(passphrase) {
    ...
    return new Promise((resolve, reject) => {
      axios
        .get(`${self.jupiter_data.server}/nxt?requestType=getAccountId&secretPhrase=${passphrase}`)
        .then((response) => {
  ...
  async attachTable(accountCredentials, tableName, currentTables = null) {
    ...
    logger.verbose(
      `##  attachTable(database=${!!accountCredentials} , tableName=${tableName}, currentTables=${currentTables})`
    )
    logger.verbose(
      '#############################################################################################################'
    )
    ...
    return new Promise((resolve, reject) => {
      ...
      eventEmitter.on('address_retrieved', async () => {
        ...
        const recipientPublicKey = accountCredentials.publicKey
          ? `&recipientPublicKey=${accountCredentials.publicKey}`
          : ''
      ...
      eventEmitter.on('tableName_obtained', () => {
        ...
        if (
          ...
          this.createNewJupiterAccount(newPassphrase)
            .then((newJupiterAccountResponse) => {
              // {address, publicKey, success}
              logger.debug('---------------------------------------------------------------------------------------')
              logger.debug(
                `-- attachTable(accountCredentials, tableName=${tableName}).createNewJupiterAccount(newPassphrase).THEN(newJupiterAccountResponse)`
              )
              logger.debug('---------------------------------------------------------------------------------------')
              logger.sensitive(`newJupiterAccountResponse= ${JSON.stringify(newJupiterAccountResponse)}`)

              if (
                newJupiterAccountResponse.success === true &&
                newJupiterAccountResponse.address &&
                newJupiterAccountResponse.address.length > 0
              ) {
                ...
                logger.error(
                  `attachTable().createNewAddress().then() error: ${JSON.stringify(newJupiterAccountResponse)}`
                )
                reject('There was an error')
              }
            })
      ...
      eventEmitter.on('verified_balance', () => {
        ...
        this.loadUserAndAppData(accountCredentials)
          .then((response) => {
            ...
            logger.verbose('---------------------------------------------------------------------------------------')
            logger.verbose(
              `-- attachTable().loadUserAndAppData(database=${!!accountCredentials}).then(response=${!!response})`
            )
            logger.verbose('---------------------------------------------------------------------------------------')
            logger.sensitive(`response=${JSON.stringify(response)}`)
  ...
  async getUnconfirmedData(address, passphrase, filter = {}, accessData) {
    ...
    try {
      response = await axios.get(
        `${self.jupiter_data.server}/nxt?requestType=getUnconfirmedTransactions&account=${address}`
      )
    } catch (e) {
    ...
    for (let x = 0; x < transactions.length; x += 1) {
      ...
      if (
        (thisTransaction.senderRS === address || thisTransaction.recipientRS === address) &&
        ...
        try {
          ...
          if (accessData) {
            unconfirmedDataObject = JSON.parse(
              self.decrypt(decryptedData.decryptedMessage, accessData.encryptionPassword)
            )
          }
  ...
  async getTransactions(filter) {
    ...
    if (!filter.noUnconfirmed) {
      try {
        rawUnconfirmedTransactions = (
          await axios.get(`${this.jupiter_data.server}/nxt?requestType=getUnconfirmedTransactions&account=${address}`)
        ).data
      } catch (e) {
  ...
  decryptFromRecord(transaction, address, passphrase) {
    ...
    return new Promise((resolve, reject) => {
      if (transaction && transaction.attachment && transaction.attachment.encryptedMessage) {
        axios
          .get(
            `${self.jupiter_data.server}/nxt?requestType=decryptFrom&secretPhrase=${passphrase}&account=${address}&data=${transaction.attachment.encryptedMessage.data}&nonce=${transaction.attachment.encryptedMessage.nonce}`
          )
          .then((response) => {
  ...
  createTable() {
    ...
    return new Promise((resolve, reject) => {
      ...
      eventEmitter.on('address_retrieved', () => {
        ...
        }/nxt?requestType=sendMessage&secretPhrase=${appAccount}&recipient=${appAccountAddress}&messageToEncrypt=${encryptedTableData}&feeNQT=${
          self.jupiter_data.feeNQT / 2
        }&deadline=${self.jupiter_data.deadline}&recipientPublicKey=${appPublickKey}&compressMessageToEncrypt=true`
      ...
      self
        ...
        .then((response) => {
          if (response.minimumAppBalance === true) {
            ...
            logger.error(
              `A minimum of ${parseFloat(
                self.jupiter_data.minimumAppBalance / 10 ** self.jupiter_data.moneyDecimals
  ...
  createAppDatabase() {
    ...
    rl.question('What is the name of the app?\n', (answer1) => {
      ...
      rl.question('Please provide an encryption password for your Jupiter data:\n', (answer2) => {
        password = answer2
        rl.question(
          'What is the URL/IP of your Jupiter server? Do not use IP unless you also use the port!\n',
          (answer) => {
            server = answer
            const currentData = {
            ...
            rl.question(
              ...
              () => {
                ...
                axios
                  .get(`${server}/nxt?requestType=getAccountId&secretPhrase=${passphrase}`)
                  .then((response) => {
                    const address = response.data.accountRS
