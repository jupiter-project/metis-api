...
class JupiterTransactionMessageService {
  ...
  constructor(jupiterAPIService, transactionUtils) {
    if (!(jupiterAPIService instanceof JupiterAPIService)) {
      throw new Error('jupiterApiServer not valid')
    }
  ...
  async putReadableMessagesIntoMessageTransactionsAndReturnTransactions(messageTransactions, accountProperties) {
    logger.verbose('#################################################')
    logger.verbose(
      `## putReadableMessagesIntoMessageTransactionsAndReturnTransactions(messageTransactions.length= ${messageTransactions.length}, accountProperties)`
    )
    logger.verbose('##')
    if (!transactionUtils.areValidTransactions(messageTransactions)) {
      throw new Error('messageTransactions is invalid')
    }
    ...
    return new Promise((resolve, reject) => {
      const transactions = _.clone(messageTransactions)
      const metisMessagePromises = []
          ...
          new Promise((res, rej) => {
            this.getReadableMessageContainerFromMessageTransactionIdAndDecrypt(
              ...
              .then((messageContainer) => {
                logger.verbose('-----------------------------------------------------')
                logger.verbose(
                  'decipherMessagesFromMessageTransactionsAndReturnTransactions(messageTransactions).getReadableMessageFromMessageTransactionIdAndDecrypt().then()'
                )
                logger.verbose('-----------------------------------------------------')
   ...
   */
  readMessagesFromMessageTransactionIdsAndDecryptOrNullAndReturnMessageContainer(
  ...
  readMessagesFromMessageTransactionIdsAndDecryptAndReturnMessageContainer(messageTransactionIds, crypto, passphrase) {
    ...
    return this.executeCallbackForEachTransactionIdInBatches(messageTransactionIds, (transactionId) =>
      this.getReadableMessageContainerFromMessageTransactionIdAndDecrypt(transactionId, crypto, passphrase)
    )
  }
   ...
   */
  async readMessagesFromMessageTransactionIdsAndDecryptOrPassThroughAndReturnMessageContainers(
    messageTransactionIds,
    crypto,
    passphrase
  ) {
    ...
    return await this.executeCallbackForEachTransactionIdInBatches(messageTransactionIds, (transactionId) =>
      this.getReadableMessageContainerFromMessageTransactionIdAndDecryptOrPassThrough(transactionId, crypto, passphrase)
    )
  }
  ...
  readMessagesFromMessageTransactionIdsAndReturnMessageContainers(messageTransactionIds, passphrase) {
    ...
    if (!transactionUtils.areWellFormedJupiterTransactionIds(messageTransactionIds)) {
      throw new Error('messageTransactionsIds is not valid')
    }
    ...
    return this.executeCallbackForEachTransactionIdInBatches(messageTransactionIds, (transactionId) =>
      this.getReadableMessageContainerFromMessageTransactionId(transactionId, passphrase)
    )
  }
  ...
  async executeCallbackForEachTransactionIdInBatches(messageTransactionIds, callback) {
    logger.verbose(
      `#### executeCallbackForEachTransactionIdInBatches(messageTransactionIds, callback): messageTransactionsIds.length= ${messageTransactionIds.length}`
    )
    ...
    if (!transactionUtils.areWellFormedJupiterTransactionIds(messageTransactionIds)) {
      throw new Error('messageTransactionIds are invalid')
    }
    try {
      const groupMaxSize = 50
      const metisMessagePromises = []
      const transactionGroupCount = Math.ceil(messageTransactionIds.length / groupMaxSize)
      let results = []
      for (let currentGroup = 0; currentGroup < transactionGroupCount; currentGroup++) {
        const transactionsCount = Math.min(messageTransactionIds.length, groupMaxSize)
  ...
  async readMessagesFromMessageTransactionsAndDecryptOrPassThroughAndReturnMessageContainers(
    transactions,
    crypto,
    passphrase
  ) {
    logger.verbose('##############################################################################################')
    logger.verbose(
      '## readMessagesFromUnconfirmedMessageTransactionsAndDecryptOrPassThroughAndReturnMessageContainers(unconfirmedTransactions, crypto, passphrase)'
    )
    logger.verbose('##')

    const transactionIds = this.transactionUtils.extractTransactionIds(transactions)

    return await this.readMessagesFromMessageTransactionIdsAndDecryptOrPassThroughAndReturnMessageContainers(
      transactionIds,
      crypto,
      passphrase
    )
  ...
  async getReadableMessageContainersFromTransactions(transactions, ownerAccountProperties, isMetisEncrypted = true) {
    logger.verbose(
      `#### getReadableMessageContainersFromTransactions(transactions, gravityAccountProperties, isMetisEncrypted= ${isMetisEncrypted})`
    )
    if (!Array.isArray(transactions)) {
      throw new mError.MetisError('transactions is not an array')
    }
    ...
    if (!(ownerAccountProperties instanceof GravityAccountProperties)) {
      throw new mError.MetisErrorBadGravityAccountProperties('ownerAccountProperties is invalid')
    }
    if (!transactionUtils.areValidTransactions(transactions)) {
    ...
    const messages = isMetisEncrypted
      ? await this.readMessagesFromMessageTransactionIdsAndDecryptOrNullAndReturnMessageContainer(
          transactionIds,
          ownerAccountProperties.crypto,
          ownerAccountProperties.passphrase
        )
      : await this.readMessagesFromMessageTransactionIdsAndReturnMessageContainers(
          transactionIds,
          ownerAccountProperties.passphrase
        )
  ...
  async getReadableContainersFromMessageTransactionIdsAndDecrypt(transactionIds, passphrase, crypto) {
    const promises = transactionIds.map((transactionId) => {
      return this.getReadableMessageContainerFromMessageTransactionIdAndDecrypt(transactionId, crypto, passphrase)
    })
  ...
  async getReadableMessageContainerFromMessageTransactionIdAndDecrypt(messageTransactionId, crypto, passphrase) {
    ...
    if (!gu.isWellFormedJupiterTransactionId(messageTransactionId)) {
      throw new Error('messageTransactionId is invalid')
    }
    if (!gu.isWellFormedPassphrase(passphrase)) {
    ...
    try {
      const decryptedMessageContainer = await this.getReadableMessageContainerFromMessageTransactionId(
      ...
      if (!messageToParse) {
        ...
        logger.warn('???????????????????????????????????????????????')
        logger.warn(
          `?? unable to mDecrypt message: ${decryptedMessageContainer.message.substring(0, 30)}  --- SIZE: ${
            decryptedMessageContainer.message.length
          }`
        )
        logger.warn('???????????????????????????????????????????????\n')
      ...
      return { message, transactionId: messageTransactionId, tag: decryptedMessageContainer.tag }
    } catch (error) {
      logger.error(
        '**** getReadableMessageContainerFromMessageTransactionIdAndDecrypt(messageTransactionId, crypto, passphrase).catch(error)'
      )
      logger.error(`error= ${error}`)
      throw error
    }
   ...
   * @returns {Promise<unknown>}
   */
  async getReadableMessageContainerFromMessageTransactionIdAndDecryptOrPassThrough(
    messageTransactionId,
    crypto,
    passphrase
  ) {
      ...
      this.getReadableMessageContainerFromMessageTransactionId(messageTransactionId, passphrase)
        .then((messageContainer) => {
          try {
            ...
            const message = gu.jsonParseOrPassThrough(messageToParse)
            return resolve({ message, transactionId: messageContainer.transactionId })
          } catch (error) {
            logger.error('********************')
            logger.error(
              '** getReadableMessageFromMessageTransactionIdAndDecrypt().getReadableMessageFromMessageTransactionId().then().catch(error)'
            )
            console.log(error)
            logger.error('**')
            return resolve('') // because of Promise.all we should not do reject.
        ...
        .catch((error) => {
          logger.error('********************')
          logger.error(
            '** getReadableMessageFromMessageTransactionIdAndDecrypt().getReadableMessageFromMessageTransactionId().catch(error)'
          )
          console.log(error)
          logger.error('**')
          return resolve('') // because of Promise.all we should not do reject.
  ...
  getReadableMessageContainerFromMessageTransactionIdOrNull(messageTransactionId, passphrase) {
    return this.getReadableMessageContainerFromMessageTransactionId(messageTransactionId, passphrase).catch(
      (error) => null
    )
  }
  ...
  getReadableMessageContainerFromMessageTransactionId(messageTransactionId, passphrase) {
    ...
    return this.jupiterAPIService.getMessage(messageTransactionId, passphrase).then((response) => {
      const message = gu.jsonParseOrPassThrough(response.data.decryptedMessage)
      return { message, transactionId: messageTransactionId, tag: response.data.message }
    })
  }
  ...
  async extractMessagesBySender(senderAccountProperties, blockChainTransactions, decipherWith = null) {
    ...
    if (!(senderAccountProperties instanceof GravityAccountProperties)) {
      throw new Error('senderAccountProperties is not GravityAccountProperties')
    }
    ...
    const messageTransactions = this.transactionUtils.filterEncryptedMessageTransactionsBySender(
    ...
    return this.readMessagesFromMessageTransactionIdsAndDecryptAndReturnMessageContainer(
      ...
      logger.verbose(
        '---- extractMessagesBySender(senderAccountProperties, blockChainTransactions, decipherWith).readMessagesFromMessageTransactionIdsAndDecrypt(filteredTransactionIds).then(messages)'
      )
  ...
  async getAllMessagesFromBlockChainAndIncludeTransactionInformation(gravityAccountProperties, blockChainTransactions) {
    logger.verbose(
      '#### getAllMessagesFromBlockChainAndIncludeTransactionInformation(accountProperties, blockChainTransactions)'
    )
    ...
    if (!blockChainTransactions) {
      throw new Error('blockChainTransactions is empty')
    }
    if (!Array.isArray(blockChainTransactions)) {
      throw new Error('blockchaintransaction not array')
    }
  ...
  fetchAllMessagesBySender(transactions, accountProperties, decipherWith = null) {
    ...
    return this.readMessagesFromMessageTransactionIdsAndDecryptAndReturnMessageContainer(
      ...
      logger.verbose('---------------------------------------------------------------------------------------')
      logger.verbose(
        'fetchAllMessagesBySender().getAllBlockChainTransactions().readMessagesFromMessageTransactionIdsAndDecrypt().then(messages)'
      )
      logger.verbose('---------------------------------------------------------------------------------------')
      logger.verbose(`messages.length ${messages.length}`)
  ...
  async sendTaggedAndEncipheredMetisMessage(
    ...
    logger.verbose(
      '#### sendTaggedAndEncipheredMetisMessage(fromPassphrase, toAddress, metisMessage, tag, feeType, recipientPublicKey, prunable )'
    )
    if (!gu.isWellFormedPassphrase(fromPassphrase)) {
      throw new Error(`fromPassphrase is not valid: ${fromPassphrase}`)
    }
    if (!gu.isWellFormedJupiterAddress(toAddress)) {
      throw new mError.MetisErrorBadJupiterAddress(`toAddress: ${toAddress}`)
    }
    ...
    const _recipientPublicKey = recipientPublicKey
