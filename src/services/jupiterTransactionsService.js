...
class JupiterTransactionsService {
  ...
  async getAllBlockChainTransactions(account) {
    ...
    return new Promise((resolve, reject) => {
      this.jupiterAPIService
        .getBlockChainTransactions(account)
        .then((response) => {
          logger.verbose('---------------------------------------------------------------------------------------')
          logger.verbose(`getAllBlockChainTransactions(account= ${account})`)
          logger.verbose('---------------------------------------------------------------------------------------')
  ...
  async putReadableMessagesIntoMessageTransactionsAndReturnTransactions(messageTransactions, accountProperties) {
    logger.verbose('#################################################')
    logger.verbose(
      `## decipherMessagesFromMessageTransactionsAndReturnTransactions(messageTransactions.length= ${messageTransactions.length}, accountProperties)`
    )
    logger.verbose('##')
    logger.verbose('##')
    ...
    return new Promise((resolve, reject) => {
      ...
      transactions.forEach((transaction) => {
        ...
        metisMessagePromises.push(
          new Promise((res, rej) => {
            this.messageService
              .getReadableMessageContainerFromMessageTransactionIdAndDecrypt(
                transactionId,
   ...
   *           >}
   */
  async fetchConfirmedAndUnconfirmedBlockChainTransactionsByTag(
    address,
    tag,
    firstIndex = null,
    lastIndex = null,
    ...
    const confirmedTransactionsPromise = this.fetchConfirmedBlockChainTransactionsByTag(
      address,
      tag,
      firstIndex,
      lastIndex
    ...
    const combinedTransactions = this.filterByBetweenTimestamp(
      confirmedTransactionsResponse,
      unconfirmendTransactionsResponse
    )
    ...
    return combinedTransactions.filter((transaction) => {
      return (
        transaction.hasOwnProperty('attachment') &&
  ...
  async getUnconfirmedTransactionsByTag(address, tag, firstIndex = null, lastIndex = null) {
    ...
    try {
      ...
      if (!response.data.hasOwnProperty('unconfirmedTransactions')) {
        throw new mError.MetisError(
          'getUnconfirmedBlockChainTransactions returned invalid response. Missing "data.unconfirmedTransactions"'
        )
      }
      if (!Array.isArray(response.data.unconfirmedTransactions)) {
        throw new mError.MetisError(
          'getUnconfirmedBlockChainTransactions returned invalid response. Not Array "data.unconfirmedTransactions"'
        )
      }
  ...
  async fetchConfirmedBlockChainTransactionsByTag(address, tag, firstIndex = null, lastIndex = null) {
    logger.verbose(
      `#### fetchConfirmedBlockChainTransactionsByTag(address=${address}, tag=${tag}, firstIndex, lastIndex)`
    )
    if (!gu.isWellFormedJupiterAddress(address)) throw new mError.MetisErrorBadJupiterAddress(`address: ${address}`)
    if (!tag) {
      throw new Error('tag is empty')
  ...
  async fetchUnconfirmedMessages(accountProperties) {
    ...
    const unconfirmedTransactionsResponse = await this.jupiterAPIService.getUnconfirmedBlockChainTransactions(
  ...
  fetchAllMessagesBySender(accountProperties, decipherWith = null) {
    ...
    return this.getAllConfirmedAndUnconfirmedBlockChainTransactions(accountProperties.address).then((transactions) =>
      this.messageService.fetchAllMessagesBySender(transactions, accountProperties, decipherWith)
    )
  }
   ...
   * @return {Promise<{message,transactionId}[]>}
   */
  async dereferenceListAndGetReadableTaggedMessageContainers(
    gravityAccountProperties,
    listTag,
    isMetisEncrypted = true
  ) {
    logger.verbose(
      '#### dereferenceListAndGetReadableTaggedMessageContainers(gravityAccountProperties, listTag, isMetisEncrypted)'
    )
    if (!(gravityAccountProperties instanceof GravityAccountProperties)) {
    ...
    try {
      const latestReferenceList = await this.fetchLatestReferenceList(
        gravityAccountProperties,
        listTag,
        isMetisEncrypted
      )
      ...
      latestReferenceList.forEach((transactionId) => {
        messages.push(
          this.messageService.getReadableMessageContainerFromMessageTransactionIdAndDecryptOrPassThrough(
            transactionId,
      ...
      logger.error('************************* ERROR ***************************************')
      logger.error(
        '* ** dereferenceListAndGetReadableTaggedMessageContainers(gravityAccountProperties, listTag, isMetisEncrypte).catch(error)'
      )
      logger.error('************************* ERROR ***************************************\n')
      logger.error(`error= ${error}`)
      throw error
  ...
  async fetchLatestReferenceList(gravityAccountProperties, listTag, isMetisEncrypted = true) {
    ...
    try {
      let listContainers = await this.getReadableTaggedMessageContainers(
        gravityAccountProperties,
        listTag,
        isMetisEncrypted
      )
