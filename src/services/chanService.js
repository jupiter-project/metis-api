...
const { channelConfig, metisConfig } = require('../config/constants')
...
class ChanService {
  ...
  async hasRecievedInvitationForChannel(sentToMemberAccountProperties, forChannelAddress) {
    ...
    if (!gu.isWellFormedJupiterAddress(forChannelAddress)) {
      throw new mError.MetisErrorBadJupiterAddress(`forChannelAddress: ${forChannelAddress}`)
    }
    // if (!gu.isWellFormedJupiterAddress(forChannelAddress)) {
    ...
    try {
      ...
      const invitationChannelAddresses = invitationContainers.map(
        (messageContainer) => messageContainer.message.channelRecord.address
      )
      if (invitationChannelAddresses.length === 0) {
        return false
      }
  ...
  async getChannelInvitationContainersSentToAccount(sentToMemberAccountProperties) {
    ...
    if (!(sentToMemberAccountProperties instanceof GravityAccountProperties)) {
      throw new Error('sentToMemberAccountProperties is not well formed')
    }
    try {
      ...
      const invitationContainersFilteredByTo = invitationContainers.filter(
        (invitationContainer) => invitationContainer.message.inviteeAddress === sentToMemberAccountProperties.address
      )
      ...
      const invitationContainersNotInExistingChannels = invitationContainersFilteredByTo.filter(
        (invitationContainer) =>
      ...
      const declinedInvitationContainers = await this.gravityService.getAllMessageContainersReferencedByList(
        sentToMemberAccountProperties,
        declinedTag,
        declinedTagList
      )
      ...
      const filteredInvitationContainers = invitationContainersNotInExistingChannels.filter(
        (invitationContainer) =>
          !declinedInvitationContainers.some(
            (declinedInvitationContainer) =>
              declinedInvitationContainer.transactionId === invitationContainer.transactionId
          )
      )
      ...
      const dedupedInvitationContainers = filteredInvitationContainers.reduce((reduced, invitationContainer) => {
        if (
          reduced.some((ic) => ic.message.channelRecord.address === invitationContainer.message.channelRecord.address)
        ) {
          return reduced
        }
  ...
  async createInvitation(channelAccountProperties, inviterAccountProperties, inviteeAddress, inviteePublicKey) {
    ...
    if (!(channelAccountProperties instanceof GravityAccountProperties)) {
      throw new mError.MetisErrorBadGravityAccountProperties('channelAccountProperties')
    }
    if (!(inviterAccountProperties instanceof GravityAccountProperties)) {
      throw new mError.MetisErrorBadGravityAccountProperties('inviterAccountProperties')
    }
    if (
      !(
        channelAccountProperties.hasOwnProperty('channelName') &&
        gu.isNonEmptyString(channelAccountProperties.channelName)
      )
    ) {
      throw new mError.MetisError('ChannelAccountProperties needs to have channelName')
    }
    if (!gu.isWellFormedJupiterAddress(inviteeAddress)) {
      throw new mError.MetisErrorBadJupiterAddress(`inviteeAddress: ${inviteeAddress}`)
    }
    if (!gu.isWellFormedPublicKey(inviteePublicKey)) {
      throw new mError.MetisErrorBadJupiterPublicKey(`inviteePublicKey: ${inviteePublicKey}`)
    }
    ...
    try {
      ...
      const inviteRecordJson = await this.generateInviteRecordJson(
        channelName,
        inviteeAddress,
        inviteePublicKey,
      ...
      const sendTaggedAndEncipheredMetisMessageResponse =
      ...
      await this.jupiterFundingService.waitForTransactionConfirmation(
        sendTaggedAndEncipheredMetisMessageResponse.transaction
      )
      ...
      logger.error('****************************************************************')
      logger.error(
        '** createInvitation(channelAccountProperties, inviterAccountProperties, inviteeAddress).catch(error)'
      )
      logger.error(`error= ${error}`)
      logger.error('****************************************************************')
      throw error
  ...
  async acceptInvitation(memberAccountProperties, channelAddress) {
    ...
    try {
      ...
      const hasInvitation = invitationContainers.some(
        (invitationContainer) => invitationContainer.message.channelRecord.address === channelAddress
      )
      if (!hasInvitation) {
        throw new Error(`Invitation Not Found: ${memberAccountProperties.address} invited to ${channelAddress}`)
      }
      // Third: Add the records to both Channel and User Accounts
      const invitationContainer = invitationContainers.find(
        (invitationContainer) => invitationContainer.message.channelRecord.address === channelAddress
      )
      if (invitationContainer === undefined) {
      ...
      const processNewMemberResponse = await this.processNewMember(
        memberAccountProperties,
        channelAccountPropertiesInvitedTo,
        'basic-member',
        invitationRecord.createdBy
      )
    } catch (error) {
      logger.error('***********************************************************************************')
  ...
  async getChannelAccountPropertiesOrNullFromChannelRecordAssociatedToMember(memberAccountProperties, channelAddress) {
    ...
    if (!(memberAccountProperties instanceof GravityAccountProperties)) {
      throw new mError.MetisErrorBadGravityAccountProperties('memberAccountProperties')
    }
    logger.sensitive(`channelAddress= ${channelAddress}`)
    ...
    try {
      ...
      const transactionsBySelf = transactionUtils.filterEncryptedMessageTransactionsBySender(
        transactions,
        memberAccountProperties.address
      )
      ...
      const channelAccountRecord = await this.fetchChannelAccountRecord(
        transactionId,
  ...
  async fetchChannelAccountRecord(transactionId, crypto, passphrase) {
    ...
    if (!(crypto instanceof GravityCrypto)) {
    ...
    const messageContainer =
      await this.jupiterTransactionsService.messageService.getReadableMessageContainerFromMessageTransactionIdAndDecrypt(
        transactionId,
        crypto,
        passphrase
      )
  ...
  fetchMultiChannelRecords(transactionIds, crypto, passphrase) {
    if (!Array.isArray(transactionIds)) {
      throw new Error('transactionIds is invalid')
    }
    if (!(crypto instanceof GravityCrypto)) {
      throw new Error('crypto is invalid')
    }
  ...
  async fundNewChannelAndAddFirstMember(channelAccountProperties, firstMemberProperties) {
    ...
    if (!(firstMemberProperties instanceof GravityAccountProperties)) {
      throw new mError.BadGravityAccountPropertiesError('firstMemberProperties')
    }
    if (!(channelAccountProperties instanceof GravityAccountProperties)) {
      throw new mError.BadGravityAccountPropertiesError('channelAccountProperties')
    }
    if (!gu.isWellFormedPublicKey(firstMemberProperties.publicKey)) {
      throw new mError.MetisErrorBadJupiterPublicKey('firstMemberProperties.publicKey')
    }
    if (!gu.isNonEmptyString(channelAccountProperties.channelName)) {
      throw new mError.MetisError('channelAccountProperties.channelName is empty')
    }
    try {
      const fundingResponse = await jupiterFundingService.provideInitialChannelAccountFunds(channelAccountProperties)
      const transactionWaitResponse = await jupiterFundingService.waitForTransactionConfirmation(
        fundingResponse.data.transaction
      ) // need to wait for confirmation in order for account to send transactions.
      const processNewMemberResponse = await this.processNewMember(
  ...
  async processNewMember(
    ...
    if (!(memberAccountProperties instanceof GravityAccountProperties)) {
      throw new BadGravityAccountPropertiesError('memberAccountProperties')
    }
    if (!(channelAccountPropertiesInvitedTo instanceof GravityAccountProperties)) {
      throw new BadGravityAccountPropertiesError('channelAccountPropertiesInvitedTo')
    }
    if (!gu.isWellFormedPublicKey(memberAccountProperties.publicKey)) {
      throw new mError.MetisErrorBadJupiterPublicKey('memberAccountProperties.publicKey')
    }
    if (!gu.isWellFormedPublicKey(channelAccountPropertiesInvitedTo.publicKey)) {
      throw new mError.MetisErrorBadJupiterPublicKey('channelAccountPropertiesInvitedTo.publicKey')
    }
    logger.sensitive(`memberAccountProperties.address= ${JSON.stringify(memberAccountProperties.address)}`)
    ...
    try {
      ...
      const response2 = await this.addMemberInfoToChannelIfDoesntExist(
        memberAccountProperties,
        channelAccountPropertiesInvitedTo,
        role
      )
      const response3 = await this.addChannelInfoToAccountIfDoesntExist(
        memberAccountProperties,
        channelAccountPropertiesInvitedTo,
        createdBy
      )
   ...
   */
  async generateInviteRecordJson(
    channelName,
    inviteeAddress,
    inviteePublicKey,
    inviterAccountProperties,
    ...
    logger.verbose(
      '#### generateInviteRecordJson(channelName, inviteeAddress, inviteePublicKey, inviterAccountProperties, channelAccountProperties)'
    )
    if (!(channelAccountProperties instanceof GravityAccountProperties)) {
      throw new mError.MetisErrorBadGravityAccountProperties('channelAccountProperties')
    }
    ...
    if (!valid.isValid) {
      ...
      logger.error('************************* ERROR ***************************************')
      logger.error(
        '* ** generateInviteRecordJson(channelName, inviteeAddress, inviteePublicKey, inviterAccountProperties, channelAccountProperties).catch(error)'
      )
      logger.error('************************* ERROR ***************************************\n')
  ...
  async generateNewChannelRecordJson(channelName, channelAccountProperties, createdByAddress) {
    ...
    if (!gu.isWellFormedJupiterAddress(createdByAddress)) {
      throw new mError.MetisErrorBadJupiterAddress(`createdByAddress: ${createdByAddress}`)
    }
    // if (!gu.isWellFormedJupiterAddress(createdByAddress)) throw new mError.BadJupiterAddressError(createdByAddress)
    if (!(channelAccountProperties instanceof GravityAccountProperties)) {
      throw new mError.MetisError('channelAccountProperties is invalid')
    }
    if (!gu.isWellFormedPublicKey(channelAccountProperties.publicKey)) {
      throw new mError.MetisErrorBadJupiterPublicKey(channelAccountProperties.publicKey)
    }
  ...
  async accountHasReferenceAccountInfo(accountProperties, referenceAddress, listTag, recordTag) {
    ...
    if (!(accountProperties instanceof GravityAccountProperties)) {
      throw new Error('accountProperties is invalid')
    }
    if (!gu.isWellFormedJupiterAddress(referenceAddress)) {
      throw new mError.MetisErrorBadJupiterAddress(`referenceAddress: ${referenceAddress}`)
    }
    ...
    const containers = await this.jupiterTransactionsService.getReadableTaggedMessageContainers(
      accountProperties,
      listTag
    )
    ...
    if (firstContainer.message.includes(firstTransaction.transaction)) {
      return true
    }
  ...
  async addChannelInfoToAccountIfDoesntExist(accountProperties, channelAccountProperties, createdBy = null) {
    ...
    if (!(channelAccountProperties instanceof GravityAccountProperties)) {
      throw new Error('invalid channelAccountProperties')
    }
  ...
  async addMemberInfoToChannelIfDoesntExist(memberProperties, channelProperties, role = 'basic-member') {
    ...
    try {
      ...
      const transactionIdForTheLatestTransactionsList =
        transactionUtils.extractTransactionIdFromTransactionResponse(transactionResponse)
      ...
      memberPublicKeys.map(async (memberKey) => {
        await this.addE2EPublicKeyToChannel(
          memberKey,
          memberProperties.address,
          channelProperties,
          memberProperties.getCurrentAliasNameOrNull()
        )
      })
  ...
  async getMemberChannels(memberProperties) {
    ...
    try {
      ...
      const transactions2 = this.transactionUtils.filterEncryptedMessageTransactionsBySender(
        transactions,
        memberProperties.address
      ) // used to be by TableChannel. we are removing the table channel.
      const transactionIds = this.transactionUtils.extractTransactionIds(transactions2)
      const multiChannelRecords = await this.fetchMultiChannelRecords(
        transactionIds,
        memberProperties.crypto,
        memberProperties.passphrase
      )
  ...
  getChannelMembers(channelAccountProperties) {
    ...
    return jupiterTransactionsService
      .dereferenceListAndGetReadableTaggedMessageContainers(channelAccountProperties, listTag)
      .then((messageContainers) =>
        uniqBy(
          messageContainers.map((messageContainer) => messageContainer.message),
  ...
  addE2EPublicKeyToChannel(userE2EPublicKey, userAddress, channelAccountProperties, userAlias) {
    ...
    return this.jupiterAccountService.addE2EPublicKeyToJupiterAccount(
      userE2EPublicKey,
      channelAccountProperties,
      userAddress,
      userAlias,
