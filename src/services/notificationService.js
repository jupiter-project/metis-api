...
module.exports = {
  ...
  incrementBadgeCounter: (filter) => {
    ...
    return Notifications.findOneAndUpdate(filter, { $inc: { 'pnAccounts.$[].badgeCounter': 1 } }, { new: true }).lean()
  },
  findOneNotificationAndRemovePNToken: (filter, provider, token) => {
    return Notifications.findOne(filter)
      ...
      .then(async (notification) => {
        ...
        const providerToken = notification.pnAccounts.find(
          (account) => account.provider === provider && account.token === token
        )
