        ...
        if (aliasCheckup.available) {
            ...
            if (aliasResponse.transaction) {
                return {
                    success: true,
                    message: 'Alias set',
                    fullResponse: aliasResponse
                };
            }
            return {
                success: false,
                message: 'Error setting alias',
    ...
    update(passphrase) {
        ...
        return new Promise(async (resolve, reject)=>{
            if (self.verify().errors === true) {
                ...
                const encryptedRecord = gravity.encrypt(JSON.stringify(fullRecord), self.record.encryption_password);
                ...
                axios.post(callUrl).then((response)=>{
                    ...
                    if (response.data.broadcasted && response.data.broadcasted === true) {
                        self.setAlias(passphrase).then((aliasSetting)=>{
