...
class Model {
    ...
    generateId(tableCredentials) {
        ...
        return new Promise((resolve, reject)=>{
            ...
            if (tableCredentials.public_key) {
                ...
                gravity.getAccountInformation(tableCredentials.passphrase).then((response)=>{
    ...
    loadAppTable(accountCredentials) {
        ...
        return new Promise((resolve, reject)=>{
            gravity.loadUserAndAppData(accountCredentials).then((response)=>{
                logger.verbose('---------------------------------------------------------------------------------------');
    ...
    getAllVersions() {
        ...
        let createdAt;
        return new Promise((resolve, reject)=>{
            gravity.getAllRecords(self.table).then((response)=>{
    ...
    validateRequest() {
        ...
        return new Promise((resolve, reject)=>{
            if (self.model === 'user') {
                ...
                gravity.findById(self.user.id, 'user').then((response)=>{
                    ...
                    reject({
                        success: false,
                        errors: 'There was an error in authentication of request/user validation'
                    });
                });
            } else {
                reject({
    ...
    loadRecords(accessData = false) {
        ...
        return new Promise((resolve, reject)=>{
            eventEmitter.on('tableData_loaded', ()=>{
                ...
                gravity.getRecords(ownerAddress, accountPropertiesHolder, accountPropertiesPassphrase, {
                    accessData,
                    size: 'all',
                    show_pending: null,
                    ...
                    return resolve({
                        success: true,
                        records: finalList,
                        records_found: finalList.length
                    });
                }).catch((err)=>{
                    logger.error('***********************************************************************************');
            ...
            eventEmitter.on('verified_request', ()=>{
                if (self.user && self.user.api_key === user.record.api_key || accessData) {
                    self.loadAppTable(accessData).then((res)=>{
                        tableData = res;
        ...
        return new Promise((resolve, reject)=>{
            if (self.verify().errors === true) {
                ...
                eventEmitter.on('id_generated', ()=>{
                    ...
                    axios.post(callUrl).then((channelDataMessageResponse)=>{
                        logger.verbose('-----------------------------------------------------------------------------------');
                        ...
                        if (channelDataMessageResponse.data.broadcasted && channelDataMessageResponse.data.broadcasted === true) {
                            ...
                            reject({
                                success: false,
                                errors: channelDataMessageResponse.data.errorDescription
                            });
                        } else {
                            reject({
                                success: false,
                ...
                eventEmitter.on('table_loaded', ()=>{
                    ...
                    .then(()=>{
                        ...
                        if (self.record.id === undefined) {
                            reject({
                                success: false,
                                errors: 'Id for model was not generated'
                            });
                        }
                ...
                eventEmitter.on('request_authenticated', ()=>{
                    ...
                    logger.verbose('-- ');
                    self.loadAppTable(accessLink).then((applicationTableCredentials)=>{
                        logger.verbose('-----------------------------------------------------------------------------------');
                ...
                eventEmitter.on('authenticate_user_request', ()=>{
                    ...
                    if (user.record.api_key === self.user.api_key) {
                        ...
                        resolve({
                            success: false,
                            errors: 'Wrong user api key in request'
                        });
                    }
                });
                if (self.model === 'user') {
                    ...
                    gravity.findById(self.user.id, 'user').then((response)=>{
                        user = new User1(response.record);
                        eventEmitter.emit('authenticate_user_request');
                    }).catch((err)=>{
                        logger.error(err);
                        reject({
            ...
            if (self.verify().errors === true) {
                ...
                eventEmitter.on('table_loaded', ()=>{
                    if (self.record.id === undefined) {
                        reject({
                            success: false,
                            errors: 'Cannot update. Id is missing from  data.'
                        });
                    }
                    eventEmitter.emit('id_verified');
                });
                eventEmitter.on('request_authenticated', ()=>{
                    self.loadAppTable().then((res)=>{
                        recordTable = res;
                        eventEmitter.emit('table_loaded');
                    }).catch((err)=>{
                        reject({
    ...
    findAll() {
        ...
        return new Promise((resolve, reject)=>{
            gravity.getAllRecords(self.table, scope).then((response)=>{
                const { records  } = response;
                const collection = {};
