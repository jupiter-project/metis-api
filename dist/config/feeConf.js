...
if (isNaN(process.env.INVITATION_TO_CHANNEL_FEE)) {
    throw new Error(`Environment Variable INVITATION_TO_CHANNEL_FEE is not a number ${process.env.INVITATION_TO_CHANNEL_FEE}`);
}
...
if (isNaN(process.env.METIS_CHANNEL_MEMBER_FEE)) {
    throw new Error(`Environment Variable METIS_CHANNEL_MEMBER_FEE is not a number ${process.env.METIS_CHANNEL_MEMBER_FEE}`);
}
...
if (isNaN(process.env.ACCOUNT_PROPERTY_DELETION_FEE)) {
    throw new Error(`Environment Variable ACCOUNT_PROPERTY_DELETION_FEE is not a number ${process.env.ACCOUNT_PROPERTY_DELETION_FEE}`);
}
...
module.exports.feeConf = {
    regularTransaction: {
        NQT: regularTransactionFee
    },
    accountRecord: {
        NQT: accountRecordFee
    },
    newUserFunding: {
        NQT: newUserFundingFee
    },
    ordinaryPayment: {
        NQT: ordinaryPaymentFee
    },
    accountProperty: {
        NQT: accountPropertyFee
    },
    accountPropertyDeletion: {
        NQT: accountPropertyDeletionFee
    },
    invitationToChannel: {
        NQT: invitationToChannelFee
    },
    channelMember: {
        NQT: metisChannelMemberFee
    },
    aliasAssignment: {
        NQT: aliasAssignmentFee
    },
    message: {
        NQT: metisMessageFee
    },
    messageCharacter: {
        NQT: messageCharacterFee
    }
};
