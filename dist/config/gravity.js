...
const { tableConfig , userConfig  } = require('./constants');
 ...
 */ class Gravity {
    constructor(){
        ...
        this.jupiter_data = {
            ...
            feeNQT: feeManagerSingleton.getFee(FeeManager.feeTypes.regular_transaction),
            deadline: process.env.JUPITER_DEADLINE,
            minimumTableBalance: fundingManagerSingleton.getFundingAmount(FundingManager.FundingTypes.new_table),
            minimumAppBalance: fundingManagerSingleton.getFundingAmount(FundingManager.FundingTypes.new_user),
    ...
    showTables(returnType = 'app') {
        ...
        return new Promise((resolve, reject)=>{
            logger.debug('-- ---- -- --- -- --- $$$$$ -- ---- -- --- -- --- 2');
            self.loadUserAndAppData().then((response)=>{
    ...
    loadTables(returnType = 'app', accessData) {
        ...
        return new Promise((resolve, reject)=>{
            logger.debug('-- ---- -- --- -- --- $$$$$ -- ---- -- --- -- --- 3');
            self.loadUserAndAppData(accessData).then((response)=>{
   ...
   */ decrypt(text, password = this.password) {
    ...
    validateTransaction(transaction, filter) {
        ...
        if (filter.confirmationsHigherThan && transaction.confirmations < filter.confirmationsHigherThan) {
            return false;
        }
        if (filter.confirmationsLowerThan && transaction.confirmations > filter.confirmationsLowerThan) {
            return false;
        }
   ...
   */ loadAccountData(accountCredentials) {
        // -> getREcords
        logger.info('###################################################################################');
        ...
        return new Promise((resolve, reject)=>{
            ...
            this.getRecords(accountCredentials.address, accountCredentials.address, accountCredentials.passphrase, {
                size: 'all',
                show_pending: null,
                show_unconfirmed: false
            }, accountCredentials.password).then((recordsContainer)=>{
                // {records,last_record,pending}
                // logger.verbose(`-----------------------------------------------------------------------------------`);
   ...
   */ loadUserAndAppData(containedDatabase = false) {
        // -> getREcords
        logger.verbose('#####################################################################################');
        ...
        return new Promise((resolve, reject)=>{
            ...
            self.getRecords(ownerAddress, accountPropertiesHolder, accountPropertiesHolderPassphrase, {
                size: 'all',
                show_pending: null,
                show_unconfirmed: false
            }, ownerPassword).then((response)=>{
    ...
    getCurrentTable(tables, tableName) {
        const allSpecificTables = tables.filter((table)=>!!table[tableName]) // {"users":{"address":"JUP-DD4P-62WU-M9AE-74LDD","passphrase":"st"},"confirmed":true}
        ;
    ...
    getUserRecord(tables) {
        const allSpecificTables = tables.filter((table)=>!!table.user_record);
        if (allSpecificTables.length == 0) {
            return null;
        }
        let userRecord = null;
        ...
        return new Promise((resolve, reject)=>{
            ...
            eventEmitter.on('database_retrieved', ()=>{
                for(let obj = 0; obj < Object.keys(database).length; obj += 1){
                    if (database[obj].attachment.encryptedMessage && database[obj].attachment.encryptedMessage.data != null && database[obj].recipientRS === address) {
            ...
            axios.get(`${self.jupiter_data.server}/nxt?requestType=getBlockchainTransactions&account=${address}&type=1`).then((response)=>{
   ...
   */ getRecords(ownerAddress, transactionSender, transactionSenderPassphrase, scope = {
        size: 'all',
        show_pending: null,
        show_unconfirmed: false,
        recipientOnly: false
        ...
        return new Promise((resolve, reject)=>{
            ...
            eventEmitter.on('check_on_pending', async ()=>{
                ...
                if (scope.show_unconfirmed) {
                    ...
                    try {
                        const unconfirmedObjects = await self.getUnconfirmedData(ownerAddress, transactionSenderPassphrase, filter, scope.accessData);
            ...
            eventEmitter.on('records_retrieved', ()=>{
                ...
                if (records.length <= 0) {
                    ...
                    Promise.all(messageResponses).then((results)=>{
                        ...
                        for(let index = 0; index < messages.length; index++){
                            try {
                                ...
                                const decryptedAndParsedMessage = JSON.parse(decryptedMessage);
            ...
            jupiterTransactionsService.getAllConfirmedAndUnconfirmedBlockChainTransactions(ownerAddress).then((transactions)=>{
    ...
    getAppRecords(appAddress, appPassphrase) {
        ...
        return new Promise((resolve, reject)=>{
            ...
            axios.get(`${self.jupiter_data.server}/nxt?requestType=getBlockchainTransactions&account=${appAddress}&type=1`).then((response)=>{
                database = response.data.transactions;
                eventEmitter.emit('database_retrieved');
            }).catch((error)=>{
                logger.error(`${error}`);
    ...
    getAllRecords(table, scope = {
        ...
        return new Promise((resolve, reject)=>{
                    ...
                    records.forEach((p)=>{
                        ...
                        axios.get(thisUrl).then((response)=>{
                            try {
                                ...
                                decrypted.public_key = recordsDetails[p] ? recordsDetails[p].recipientPublicKey : null;
                                decryptedRecords.push(decrypted);
                            } catch (e) {
                                logger.error(e);
                            }
            ...
            eventEmitter.on('table_access_retrieved', ()=>{
                axios.get(`${self.jupiter_data.server}/nxt?requestType=getBlockchainTransactions&account=${recordTable.address}&type=1`).then((response)=>{
            ...
            logger.debug('-- ---- -- --- -- --- $$$$$ -- ---- -- --- -- --- 4');
            self.loadUserAndAppData(scope.containedDatabase).then((res)=>{
   ...
   */ getUser(account, passphrase, containedDatabase = null) {
        ...
        return new Promise((resolve, reject)=>{
            if (account === process.env.APP_ACCOUNT_ADDRESS) {
                ...
                // ------------------------------------------------------------------------------------------------------------------------------
                this.getRecords(process.env.APP_ACCOUNT_ADDRESS, process.env.APP_ACCOUNT_ADDRESS, process.env.APP_ACCOUNT, {
                    size: 'all',
                    show_pending: null,
                    show_unconfirmed: false,
                    recipientOnly: false
                }, process.env.ENCRYPT_PASSWORD).then((applicationRecordsResponse)=>{
                    logger.debug('---------------------------------------------------------------------------------------');
                    ...
                    // ------------------------------------------------------------------------------------------------------------------------------
                    this.getRecords(account, applicationUsersTableAddress, passphrase, {
                        size: 'all',
                        show_pending: null,
                        show_unconfirmed: false,
                        recipientOnly: false
                    }, process.env.ENCRYPT_PASSWORD).then((applicationAccountUsersTableRecordsResponse)=>{
                        logger.debug('---------------------------------------------------------------------------------------');
                        ...
                        // ------------------------------------------------------------------------------------------------------------------------------
                        self.retrieveAccountTables(containedDatabase, applicationUsersTableAddress).then((userAccountTablesResponse)=>{
                            logger.debug('---------------------------------------------------------------------------------------');
                ...
                self.retrieveUserFromApp(account, passphrase).then((response)=>{
   ...
   */ retrieveAccountTables(accessData) {
        ...
        return new Promise((resolve, reject)=>{
            ...
            this.loadUserAndAppData(accessData).then((userAndAppData)=>{
   ...
   */ retrieveUserFromApp(userAccount, passphrase) {
        ...
        return new Promise((resolve, reject)=>{
            ...
            eventEmitter.on('set_responseData', ()=>{
                ...
                if (appAcctUserTableDecryptedMessagesSentToUser[0] === undefined || appAcctUserTableDecryptedMessagesSentToUser[0].user_record === undefined) {
                    return resolve({
                        error: true,
            ...
            eventEmitter.on('table_access_retrieved', ()=>{
                ...
                axios.get(`${self.jupiter_data.server}/nxt?requestType=getBlockchainTransactions&account=${applicationAccountUserTable.address}&type=1${includeExpiredPrunable}`).then((response)=>{
            ...
            self.loadUserAndAppData().then((res)=>{
                logger.debug('---------------------------------------------------------------------------------------');
    ...
    findById(id, model) {
        ...
        return new Promise((resolve, reject)=>{
          ...
          if (allRecords[x].id && allRecords[x].id === id) {
            ...
            self.getAllRecords(record.table).then((response)=>{
                allRecords = response.records;
                eventEmitter.emit('records_retrieved');
            }).catch((error)=>{
                logger.error(`${error}`);
    ...
    getBalance(address, accountId, jupServ) {
        ...
        return new Promise((resolve, reject)=>{
            ...
            eventEmitter.on('account_retrieved', ()=>{
                axios.post(`${server}/nxt?requestType=getBalance&account=${account}`).then((response)=>{
            ...
            if (!accountId) {
                axios.get(`${server}/nxt?requestType=getAccountId&secretPhrase=${addressOwner}`).then((response)=>{
    ...
    sendMoneyAndWait(recipient, transferAmount, sender, secondsToWait = 50) {
        ...
        return new Promise((resolve, reject)=>{
            this.sendMoney(recipient, transferAmount, sender).then((response)=>{
                setTimeout(async ()=>{
   ...
   */ async getAlias(aliasName) {
        ...
        logger.verbose('##');
        if (!aliasName) {
            throw new Error('aliasName is empty');
        }
   ...
   */ async getFundingMonitor(params = {}) {
    ...
    async sendMessage(data, passphrase, recipientRS, recipientPublicKey, config = {
        appEncryption: false,
   ...
   */ createNewJupiterAccount(passphrase) {
        return new Promise((resolve, reject)=>{
            axios.get(`${this.jupiter_data.server}/nxt?requestType=getAccountId&secretPhrase=${passphrase}`).then((response)=>{
    ...
    getAccountInformation(passphrase) {
        ...
        const self = this;
        return new Promise((resolve, reject)=>{
            axios.get(`${self.jupiter_data.server}/nxt?requestType=getAccountId&secretPhrase=${passphrase}`).then((response)=>{
   ...
   */ async attachTable(accountCredentials, tableName, currentTables = null) {
        ...
        return new Promise((resolve, reject)=>{
            ...
            eventEmitter.on('table_created', ()=>{
                ...
                // sendMoney(recipient, transferAmount, sender) {
                this.sendMoney(newTableAddress, process.env.JUPITER_MININUM_TABLE_BALANCE).then((response)=>{
                    logger.verbose('------------------------------------------------------------');
                ...
                // });
                    ...
                    reject({
                        success: false,
                        message: 'Unable to send Jupiter to new table newTableAddress'
                    });
                });
            });
            eventEmitter.on('address_retrieved', async ()=>{
                ...
                const encryptedTableData = this.encrypt(JSON.stringify(tableNamesContainer), accountCredentials.encryptionPassword);
                ...
                switch(tableName){
                    case 'channels':
                        tag = tableConfig.channelsTable;
                        break;
                    case 'invites':
                        tag = tableConfig.invitesTable;
                        break;
                    case 'storage':
                        tag = tableConfig.storageTable;
                        break;
                    case 'users':
                        tag = tableConfig.usersTable;
                        break;
                }
                if (!tag) {
                    throw new Error(`theres a problem with attaching the table: ${tableName}`);
                }
                logger.sensitive(`tableName= ${JSON.stringify(tableName)}`);
                logger.sensitive(`tag= ${JSON.stringify(tag)}`);
                const sendRecordResponse = await jupiterTransactionsService.messageService.sendTaggedAndEncipheredMetisMessage(accountCredentials.passphrase, accountCredentials.account, encryptedData, tag, FeeManager.feeTypes.account_record, accountCredentials.publicKey);
                ...
                if (sendRecordResponse.broadcasted) {
                    const sendTableListResponse = await jupiterTransactionsService.messageService.sendTaggedAndEncipheredMetisMessage(accountCredentials.passphrase, accountCredentials.account, encryptedTableData, tableConfig.tableList, FeeManager.feeTypes.account_record, accountCredentials.publicKey);
            ...
            eventEmitter.on('tableName_obtained', ()=>{
                ...
                if ((this.tables.indexOf(tableName) >= 0 || listOfTableNames.indexOf(tableName) >= 0) && tableInCurrentTableList) {
                    ...
                    this.createNewJupiterAccount(newPassphrase).then((newJupiterAccountResponse)=>{
                        // {address, publicKey, success}
                        logger.debug('---------------------------------------------------------------------------------------');
            ...
            eventEmitter.on('verified_balance', ()=>{
                ...
                this.loadUserAndAppData(accountCredentials).then((response)=>{
                    //  {numberOfRecords,success,app:{tables,appData,address}, message,tables,hasUserTable,userRecord}
                    logger.verbose('---------------------------------------------------------------------------------------');
   ...
   */ extractTableNamesFromTables(tables) {
    ...
    async getUnconfirmedData(address, passphrase, filter = {}, accessData) {
        ...
        for(let x = 0; x < transactions.length; x += 1){
            ...
            if ((thisTransaction.senderRS === address || thisTransaction.recipientRS === address) && (thisTransaction.senderRS === filter.account || thisTransaction.recipientRS === filter.account || !filter.account)) {
                const dataObject = {
                ...
                try {
                    ...
                    if (accessData) {
                        unconfirmedDataObject = JSON.parse(self.decrypt(decryptedData.decryptedMessage, accessData.encryptionPassword));
                    }
    ...
    async getTransactions(filter) {
        ...
        if (!filter.noConfirmed) {
            ...
            for(let x1 = 0; x1 < transactions.length; x1 += 1){
                const thisTransaction1 = transactions[x1];
                thisTransaction1.confirmed = true;
                if (self.validateTransaction(thisTransaction1, filter)) {
                    validTransactions.push(thisTransaction1);
                }
            }
        }
    ...
    async decryptSingleTransaction(thisTransaction, filter) {
        ...
        if (!filter.blockchainEncryptionDisabled && thisTransaction.confirmed) {
            try {
                // eslint-disable-next-line no-await-in-loop
                decryptedData = await this.decryptMessage(thisTransaction.transaction, encryptionPassphrase);
        ...
        if (filter.includeUnconfirmed && !thisTransaction.confirmed) {
            ...
            try {
                // eslint-disable-next-line no-await-in-loop
                const response = await this.decryptFromRecord(thisTransaction, targetAccount, encryptionPassphrase);
        ...
        if (decryptedData) {
            try {
                unEncryptedData = this.decrypt(decryptedData.decryptedMessage, encryptionPassword);
                encryptionLevel = 'channel';
            } catch (e2) {
                logger.error(e2);
                logger.error('Error: Gravity file, line 2147, failed to decrypt messagee');
                try {
                    unEncryptedData = this.decrypt(decryptedData.decryptedMessage, process.env.ENCRYPT_PASSWORD);
                    encryptionLevel = 'app';
                } catch (err) {
                    logger.error(e2);
            ...
            try {
                unEncryptedData = this.decrypt(thisTransaction.attachment.message, encryptionPassword);
            } catch (e3) {
                logger.error(e3);
    ...
    async getDataTransactions(filter) {
        ...
        if (typeof filter === 'string') {
            ...
            return {
                error: true,
                message: 'Invalid account or filter data'
            };
        }
        ...
        if (!transactions.error) {
            ...
            if (dataTransactions.length > 1) {
                ...
                return _.orderBy(dataTransactions, [
    ...
    decryptFromRecord(transaction, address, passphrase) {
        ...
        return new Promise((resolve, reject)=>{
            if (transaction && transaction.attachment && transaction.attachment.encryptedMessage) {
                axios.get(`${self.jupiter_data.server}/nxt?requestType=decryptFrom&secretPhrase=${passphrase}&account=${address}&data=${transaction.attachment.encryptedMessage.data}&nonce=${transaction.attachment.encryptedMessage.nonce}`).then((response)=>{
    ...
    createTable() {
        ...
        return new Promise((resolve, reject)=>{
            ...
            eventEmitter.on('table_created', ()=>{
                ...
                // able to record information
                self.sendMoney(address).then((response)=>{
            ...
            eventEmitter.on('address_retrieved', ()=>{
                ...
                axios.post(tableListUpdateUrl).then((response)=>{
                    if (response.data.broadcasted && response.data.broadcasted === true) {} else if (response.data.errorDescription != null) {
                        logger.info('There was an Error');
                        logger.info(response.data);
                        logger.error(`Error:${response.data.errorDescription}`);
                        logger.error(response.data);
            ...
            eventEmitter.on('tableName_obtained', ()=>{
                if (self.tables.indexOf(tableName) >= 0 || tableList.indexOf(tableName) >= 0) {
                    ...
                    self.createNewJupiterAccount(passphrase).then((response)=>{
            ...
            eventEmitter.on('verified_balance', ()=>{
                if (appAccount === undefined || appAccount === '' || appAccount == null) {
                    ...
                    logger.debug('-- ---- -- --- -- --- $$$$$ -- ---- -- --- -- --- 8');
                    self.loadUserAndAppData().then(async (response)=>{
            ...
            rl.question('Please provide an encryption password for your Jupiter data:\n', (answer2)=>{
                ...
                rl.question('What is the URL/IP of your Jupiter server? Do not use IP unless you also use the port!\n', (answer)=>{
                    ...
                    rl.question("You are about to create a Jupiter account which will hold your Gravity app's data. Is the information provided above accurate? If so, press ENTER. If not, press CTRL+C to cancel and rerun command.\n", ()=>{
                        ...
                        axios.get(`${server}/nxt?requestType=getAccountId&secretPhrase=${passphrase}`).then((response)=>{
                            const address = response.data.accountRS;
