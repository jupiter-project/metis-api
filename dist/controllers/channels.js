...
const { generateNewMessageRecordJson , sendMessagePushNotifications , createMessageRecord  } = require('../services/messageService');
...
module.exports = (app, passport, jobs, websocket)=>{
   ...
   */ app.post('/v1/api/channels/call', async (req, res)=>{
        ...
        try {
            ...
            getPNTokensAndSendPushNotification([
                recipientAddress
            ], senderAlias, {}, message, 'call', metadata);
            res.send({
                success: true,
                url
            });
   ...
   */ app.post('/v1/api/channel/invite/accept', async (req, res)=>{
        ...
        chanService.acceptInvitation(memberAccountProperties, channelAddress).then(()=>{
   ...
   */ app.get('/v1/api/channels/:channelAddress/messages', async (req, res)=>{
        ...
        if (isNaN(_pageNumber)) {
            return res.status(StatusCode.ClientErrorBadRequest).send({
                message: 'pageNumber needs to be an integer'
            });
        }
        if (isNaN(_pageSize)) {
            return res.status(StatusCode.ClientErrorBadRequest).send({
                message: 'pageSize needs to be an integer'
            });
        }
        ...
        if (!(pageSize > 0 && pageSize < 1000)) {
            return res.status(StatusCode.ClientErrorBadRequest).send({
                message: 'pageSize can only be between 1 and 1000'
            });
        }
        if (pageNumber < 0) {
            return res.status(StatusCode.ClientErrorBadRequest).send({
                message: 'pageNumber needs to be more than 0'
            });
        }
        try {
            ...
            if (!channelAccountProperties) {
                return res.status(StatusCode.ServerErrorInternal).send({
                    message: `channel is not available: ${channelAddress}`
                });
            }
   ...
   */ app.post('/v1/api/channels/:channelAddress/messages', async (req, res)=>{
        ...
        try {
            const { user  } = req;
            const { message , replyMessage , replyRecipientAlias , replyRecipientAddress , attachmentObj =null , version , mentions =[] , messageType ='message'  } = req.body;
            const { channelAddress  } = req.params;
            ...
            if (!gu.isWellFormedJupiterAddress(channelAddress)) {
                return res.status(StatusCode.ClientErrorBadRequest).send({
                    message: 'Must include a valid address'
                });
            }
            if (!message) {
                return res.status(StatusCode.ClientErrorBadRequest).send({
                    message: 'Must include a valid message'
                });
            }
            if (!Array.isArray(mentions)) {
                return res.status(StatusCode.ClientErrorBadRequest).send({
                    message: 'mentions should be an array'
                });
            }
            ...
            const messageRecord = generateNewMessageRecordJson(memberAccountProperties, message, messageType, replyMessage, replyRecipientAlias, replyRecipientAddress, attachmentObj, version);
            const channelAccountProperties = await chanService.getChannelAccountPropertiesOrNullFromChannelRecordAssociatedToMember(memberAccountProperties, channelAddress);
            if (!channelAccountProperties) {
                return res.status(StatusCode.ClientErrorBadRequest).send({
                    message: 'Invalid channel address.',
                    code: MetisErrorCode.MetisError
                });
            }
            await createMessageRecord(memberAccountProperties, channelAccountProperties, message, messageType, replyMessage, replyRecipientAlias, replyRecipientAddress, attachmentObj, version);
            if (messageType === 'invitation') websocket.of('/chat').to(channelAddress).emit('newMemberChannel');
            websocket.of(websocketConstants.invitation.chat.namespace).to(channelAddress).emit(websocketConstants.invitation.chat.rooms.createMessage, {
            ...
            logger.error(JSON.stringify(error1));
            return res.status(StatusCode.ServerErrorInternal).send({
                message: `Error sending message. ${error1.message}`,
                code: error1.code
            });
        }
    });
   ...
   */ app.get('/v1/api/channel/invites', async (req, res)=>{
        ...
        try {
            ...
            const memberAccountProperties = await instantiateGravityAccountProperties(req.user.passphrase, req.user.password);
            await chanService.getChannelInvitationContainersSentToAccount(memberAccountProperties).then((channelInvitations)=>{
   ...
   */ app.post('/v1/api/channel/invite', async (req, res)=>{
        ...
        if (!gu.isWellFormedJupiterAddress(channelAddress)) {
            return res.status(StatusCode.ClientErrorBadRequest).send({
                message: 'channelAddress is invalid',
                code: MetisErrorCode.MetisErrorBadJupiterAddress
            });
        }
        try {
            ...
            const newInvitation = await chanService.createInvitation(channelAccountProperties, inviterAccountProperties, inviteeAddress, inviteePublicKey);
   ...
   */ app.post('/v1/api/channel', async (req, res, next)=>{
        ...
        if (!gu.isNonEmptyString(channelName)) {
            return res.status(StatusCode.ClientErrorBadRequest).send({
                message: 'Need channelName in body'
            });
        }
        ...
        const job = jobs.create('channel-creation-confirmation', {
            channelAccountProperties,
            memberAccountProperties
        }).priority('high').removeOnComplete(false).save((error)=>{
