...
module.exports = (app, passport)=>{
    ...
    app.get('/v1/api/balance', (req, res)=>{
        ...
        if (!user.address) {
            return res.status(400).send({
                message: 'User account not provided'
            });
        }
        ...
        const appAccountProperties = new ApplicationAccountProperties(DEADLINE, STANDARD_FEE, ACCOUNT_CREATION_FEE, TRANSFER_FEE, MINIMUM_TABLE_BALANCE, MINIMUM_APP_BALANCE, MONEY_DECIMALS);
        const jupiterAPIService = new JupiterAPIService(process.env.JUPITERSERVER, appAccountProperties);
        jupiterAPIService.getBalance(user.address).then((response)=>{
            if (response && response.data.unconfirmedBalanceNQT) {
                return res.status(200).send({
                    balance: response.data.unconfirmedBalanceNQT / 100000000
                });
            }
    ...
    app.get('/v1/api/recent-transactions', (req, res)=>{
        ...
        jupiterAPIService.get(params).then((response)=>{
    ...
    app.post('/v1/api/transfer-money', async (req, res)=>{
        ...
        const toJupiterAccount = {
            address: recipient
        };
        // TODO make the "transferMoney" function static in order to avoid generating all unnecessary properties
        jupiterAPIService.transferMoney(fromJupAccount, toJupiterAccount, +amount, fee).then(()=>res.status(200).send({
                message: 'Transfer successfully executed'
            })).catch((error)=>{
            logger.error('Error with the transfer' + `${error}`);
            res.status(500).send({
                message: 'There was an issue transferring money, please try again later',
                error: `${error}`
            });
        });
    });
