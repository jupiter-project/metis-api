...
const arraysEqual = (a1, a2)=>{
    ...
    /* WARNING: arrays must not contain {objects} or behavior may be undefined */ return JSON.stringify(a1) === JSON.stringify(a2);
};
 ...
 */ const ipLogger = function(jupAddress, alias, ipAddress) {
    ...
    NewAccountIp.create(newAccountIp).then((result)=>console.log('NewAccountIp successfully created', result)).catch((error)=>logger.error(`Error saving logger record ${error}`));
    ipLoggerCleanUp();
};
 ...
 */ const ipLoggerCleanUp = function() {
    ...
    NewAccountIp.deleteMany({
        timestamp: {
            $lt: sinceDate
        }
    }).then((result)=>logger.info('NewAccountIp successfully removed')).catch((error)=>logger.error(`Error cleaning up logger record ${error}`));
};
module.exports = {
    isObject,
