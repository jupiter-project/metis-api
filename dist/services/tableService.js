 ...
 */ class GravityTablesService {
   ...
   */ async postTableListRecord(databaseGravityAccountType, encryptedTableListRecord) {
        return new Promise((resolve, reject)=>{
            ...
            this.jupiterAccountService.postSimpleMessage(databaseGravityAccountType, encryptedTableListRecord, true, feeNQT).then((res)=>{
   ...
   */ async attachTable(database, nameOfTableToAttach, currentTables = []) {
        ...
        return new Promise((resolve, reject)=>{
            this.applicationTransactions.getAccountStatement().then((accountStatement)=>{
                logger.verbose('-----------------------------------------------------------------------------------');
                ...
                .then((userAccountData)=>{
                    ...
                    this.setUpNewGravityAccount(database.address, database.publicKey, nameOfTableToAttach, userAccountTableNames, passphrase, database.encryptionPassword, this.applicationAccountInfo.algorithm).then((response)=>{
                        return resolve(response);
                    });
                }).catch((error)=>{
                    logger.error(`${error}`);
...
class TableService {
   ...
   */ async createTableListRecord(accountProperties, arrayOfTableNames) {
        ...
        ;
        return new Promise((resolve, reject)=>{
            this.jupiterApiService.sendSimpleEncipheredMetisMessage(accountProperties, accountProperties, encrypted, fee, subtype, false).then((response)=>{
                // @TODO use extractTransactionId()
                resolve({
                    responseData: response.data,
                    transactionReport: {
                        name: 'create-table-list-record',
