...
class JupiterTransactionMessageService {
   ...
   */ async putReadableMessagesIntoMessageTransactionsAndReturnTransactions(messageTransactions, accountProperties) {
        ...
        return new Promise((resolve, reject)=>{
            ...
            transactions.forEach((transaction)=>{
                ...
                metisMessagePromises.push(new Promise((res, rej)=>{
                    this.getReadableMessageContainerFromMessageTransactionIdAndDecrypt(transactionId, accountProperties.passphrase.crypto, accountProperties.passphrase).then((messageContainer)=>{
                        logger.verbose('-----------------------------------------------------');
            ...
            Promise.all(metisMessagePromises).then((transactions)=>{
   ...
   */ readMessagesFromMessageTransactionIdsAndDecryptOrNullAndReturnMessageContainer(messageTransactionIds, crypto, passphrase) {
        return this.readMessagesFromMessageTransactionIdsAndDecryptAndReturnMessageContainer(messageTransactionIds, crypto, passphrase).then((messages)=>messages).catch((error)=>null);
    }
    /**
   *
   ...
   */ readMessagesFromMessageTransactionIdsAndDecryptAndReturnMessageContainer(messageTransactionIds, crypto, passphrase) {
        ...
        return this.executeCallbackForEachTransactionIdInBatches(messageTransactionIds, (transactionId)=>this.getReadableMessageContainerFromMessageTransactionIdAndDecrypt(transactionId, crypto, passphrase));
    }
    /**
   *
   * @param messageTransactionIds
   ...
   */ async readMessagesFromMessageTransactionIdsAndDecryptOrPassThroughAndReturnMessageContainers(messageTransactionIds, crypto, passphrase) {
        ...
        return await this.executeCallbackForEachTransactionIdInBatches(messageTransactionIds, (transactionId)=>this.getReadableMessageContainerFromMessageTransactionIdAndDecryptOrPassThrough(transactionId, crypto, passphrase));
    }
    /**
   *
   * @param messageTransactionIds
   ...
   */ readMessagesFromMessageTransactionIdsAndReturnMessageContainers(messageTransactionIds, passphrase) {
        ...
        logger.verbose(`messageTransactionIds.length=${messageTransactionIds.length}`);
        return this.executeCallbackForEachTransactionIdInBatches(messageTransactionIds, (transactionId)=>this.getReadableMessageContainerFromMessageTransactionId(transactionId, passphrase));
    }
    /**
   * Throughling
   ...
   */ async getReadableMessageContainersFromTransactions(transactions, ownerAccountProperties, isMetisEncrypted = true) {
        ...
        const transactionIds = this.transactionUtils.extractTransactionIds(transactions);
        const messages = isMetisEncrypted ? await this.readMessagesFromMessageTransactionIdsAndDecryptOrNullAndReturnMessageContainer(transactionIds, ownerAccountProperties.crypto, ownerAccountProperties.passphrase) : await this.readMessagesFromMessageTransactionIdsAndReturnMessageContainers(transactionIds, ownerAccountProperties.passphrase);
   ...
   */ async getReadableMessageContainerFromMessageTransactionIdAndDecrypt(messageTransactionId, crypto, passphrase) {
        ...
        try {
            ...
            const messageToParse = decryptedMessageContainer.tag.includes(`.${metisConfig.evm}`) ? crypto.decryptOrNullGCM(decryptedMessageContainer.message) : crypto.decryptOrNull(decryptedMessageContainer.message);
            if (!messageToParse) {
                console.log('\n');
                logger.warn('???????????????????????????????????????????????');
            ...
            return {
                ...
                tag: decryptedMessageContainer.tag
            };
        } catch (error) {
   ...
   */ async getReadableMessageContainerFromMessageTransactionIdAndDecryptOrPassThrough(messageTransactionId, crypto, passphrase) {
        ...
        return new Promise((resolve, reject)=>{
            this.getReadableMessageContainerFromMessageTransactionId(messageTransactionId, passphrase).then((messageContainer)=>{
                try {
                    ...
                    const messageToParse = messageContainer.tag.includes(`.${metisConfig.evm}`) ? crypto.decryptOrPassThroughGCM(messageContainer.message) : crypto.decryptOrPassThrough(messageContainer.message);
                    if (!messageToParse) {
                    ...
                    return resolve({
                        message,
                        transactionId: messageContainer.transactionId
                    });
                } catch (error) {
                    logger.error('********************');
    ...
    getReadableMessageContainerFromMessageTransactionIdOrNull(messageTransactionId, passphrase) {
        return this.getReadableMessageContainerFromMessageTransactionId(messageTransactionId, passphrase).catch((error)=>null);
    }
    /**
   *
   * @param messageTransactionId
   ...
   */ async extractMessagesBySender(senderAccountProperties, blockChainTransactions, decipherWith = null) {
        ...
        return this.readMessagesFromMessageTransactionIdsAndDecryptAndReturnMessageContainer(filteredTransactionIds, crypto, senderAccountProperties.passphrase).then((messageContainers)=>{
   ...
   */ async getAllMessagesFromBlockChainAndIncludeTransactionInformation(gravityAccountProperties, blockChainTransactions) {
        ...
        const messages = this.readMessagesFromMessageTransactionIdsAndDecryptOrPassThroughAndReturnMessageContainers(filteredTransactionIds, gravityAccountProperties.crypto, gravityAccountProperties.passphrase);
        return messages;
    }
    /**
   *
   ...
   */ fetchAllMessagesBySender(transactions, accountProperties, decipherWith = null) {
        ...
        const filteredTransactionIds = this.transactionUtils.extractTransactionIds(messageTransactions);
        return this.readMessagesFromMessageTransactionIdsAndDecryptAndReturnMessageContainer(filteredTransactionIds, crypto, accountProperties.passphrase).then((messages)=>{
            logger.verbose('---------------------------------------------------------------------------------------');
   ...
   */ async sendTaggedAndEncipheredMetisMessage(fromPassphrase, toAddress, metisMessage, tag, feeType, recipientPublicKey, prunable = false) {
        ...
        const response = await this.jupiterAPIService.sendMetisMessageOrMessage(JupiterAPIService.RequestType.SendMetisMessage, toAddress, _recipientPublicKey, fromPassphrase, null, fee, process.env.JUPITER_DEADLINE, null, null, tag, null, null, metisMessage, true, null, null, prunable, true, null, null, null, null, null, subtype);
        return response.data;
    }
}
module.exports.JupiterTransactionMessageService = JupiterTransactionMessageService;
module.exports.jupiterTransactionMessageService = new JupiterTransactionMessageService(jupiterAPIService, transactionUtils);
