...
const generateNewMessageRecordJson = (senderAccountProperties, message, messageType = 'message', replyMessage = null, replyRecipientAlias = null, replyRecipientAddress = null, attachmentObj = null, version = '1.0')=>{
 ...
 */ const createMessageRecord = async (fromAccountProperties, toAccountProperties, message, type, replyMessage, replyRecipientAlias, replyRecipientAddress, attachmentUrl, version)=>{
    try {
        const messageRecord = generateNewMessageRecordJson(fromAccountProperties, message, type, replyMessage, replyRecipientAlias, replyRecipientAddress, attachmentUrl, version);
        ...
        return jupiterTransactionMessageService.sendTaggedAndEncipheredMetisMessage(fromAccountProperties.passphrase, toAccountProperties.address, messageRecordString, tag, feeType, toAccountProperties.publicKey);
    } catch (error) {
 ...
 */ const sendMessagePushNotifications = async (senderAccountProperties, channelAccountProperties, mentions = [])=>{
    ...
    try {
        ...
        const pnBody = senderAlias ? `${senderAlias} has sent a message` : `${senderAccountProperties.address} has sent a message`;
        const pnTitle = senderAlias ? `${senderAlias}` : `${senderAccountProperties.address}`;
        await getPNTokensAndSendPushNotification(channelMemberAddresses, [
            channelAccountProperties.address
        ], pnBody, pnTitle, {
            channelAddress: channelAccountProperties.address
        });
        mentions.map((mentionedMemberAddress)=>{
            ...
            return getPNTokensAndSendPushNotification(mentionedMemberAddress, [
                channelAccountProperties.address
            ], body, title, {
                channelAddress
            });
