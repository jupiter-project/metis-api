"use strict";
const { findMutedChannels , updateBadgeCounter , findOneNotificationAndUpdate , findOneNotificationAndRemovePNToken , upsertNotificationDocumentWithNewToken  } = require('./notificationService');
   ...
   */ addTokenNotification: async (req, res)=>{
        try {
            ...
            if (!gu.isWellFormedJupiterAddress(userAddress)) {
                const message = {
                    message: `JupId is not valid: ${userAddress}`
                };
            ...
            if (!(provider && token)) {
                ...
                return res.status(StatusCode.ClientErrorBadRequest).json(message1);
            }
            const notification = await upsertNotificationDocumentWithNewToken(userAddress, provider, token);
            return res.json(notification);
        } catch (error) {
            const message2 = {
                message: 'Problem with addTokenNotification'
            };
            logger.error(`${error}`);
            return res.status(StatusCode.ServerErrorInternal).json(message2);
        }
    },
    deleteTokenPushNotification: (req, res)=>{
        ...
        findOneNotificationAndRemovePNToken(filter, provider, token).then(()=>res.json({
                success: true
            })).catch((error)=>{
            logger.error(`${error}`);
            res.status(500).json({
    ...
    editMutedChannels: (req, res)=>{
        ...
        const update = isMuted ? {
            $pull: {
                mutedChannelAddressList: channelAddress
            }
        } : {
            $push: {
                mutedChannelAddressList: channelAddress
        ...
        findOneNotificationAndUpdate(filter, update).then((notification)=>res.json({
                success: true,
                notification
            })).catch((error)=>{
            logger.error(`${error}`);
    ...
    findMutedChannels: (req, res)=>{
        ...
        findMutedChannels(userAddress).then(([response])=>{
            const { mutedChannelAddressList  } = response || {
    ...
    setBadgePnCounter: (req, res)=>{
        ...
        updateBadgeCounter(userAddress, badge || 0).then((response)=>res.json({
                success: true,
                response
            })).catch((error)=>{
