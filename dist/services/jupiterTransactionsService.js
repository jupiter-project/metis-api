/* eslint-disable promise/param-names */ "use strict";
...
class JupiterTransactionsService {
   ...
   */ async getAllBlockChainTransactions(account) {
        ...
        return new Promise((resolve, reject)=>{
            this.jupiterAPIService.getBlockChainTransactions(account).then((response)=>{
                logger.verbose('---------------------------------------------------------------------------------------');
                logger.verbose(`getAllBlockChainTransactions(account= ${account})`);
                logger.verbose('---------------------------------------------------------------------------------------');
   ...
   */ async getReadableTaggedMessageContainers(gravityAccountProperties, tag, isMetisEncrypted = true, firstIndex = null, lastIndex = null, transactionFilterCallback = null) {
        console.log('\n');
        ...
        if (!gu.isNonEmptyString(tag)) {
            throw new MetisError('tag is invalid');
        }
        ...
        const transactions = await this.fetchConfirmedAndUnconfirmedBlockChainTransactionsByTag(gravityAccountProperties.address, tag, firstIndex, lastIndex);
   ...
   */ async getReadableMessageContainersBySharedKey(transactionId, sharedKey, tag, isEncrypted = false) {
        ...
        logger.verbose('########################################################################\n');
        if (!gu.isNonEmptyString(transactionId)) {
            throw new MetisError('transactionId is invalid');
        }
        if (!gu.isNonEmptyString(sharedKey)) {
            throw new MetisError('sharedKey is invalid');
        }
        if (!gu.isNonEmptyString(tag)) {
            throw new MetisError('tag is invalid');
        }
        ...
        return tag.includes(`.${metisConfig.evm}`) ? crypto.decryptAndParseOrNullGCM(transaction.decryptedMessage) : crypto.decryptAndParseOrNull(transaction.decryptedMessage);
    }
    /**
   *
        ...
        return new Promise((resolve, reject)=>{
            ...
            transactions.forEach((transaction)=>{
                ...
                metisMessagePromises.push(new Promise((res, rej)=>{
                    this.messageService.getReadableMessageContainerFromMessageTransactionIdAndDecrypt(transactionId, accountProperties.passphrase.crypto, accountProperties.passphrase).then((messageContainer)=>{
                        logger.verbose('-----------------------------------------------------');
   ...
   */ async fetchConfirmedAndUnconfirmedBlockChainTransactionsByTag(address, tag, firstIndex = null, lastIndex = null, orderBy = 'desc') {
        ...
        const [confirmedTransactionsResponse, unconfirmendTransactionsResponse] = await Promise.all([
            confirmedTransactionsPromise,
            unconfirmedTransactionsPromise
        ]);
        ...
        return combinedTransactions.filter((transaction)=>{
            return transaction.hasOwnProperty('attachment') && transaction.attachment.hasOwnProperty('message') && transaction.attachment.message.includes(tag);
        });
    }
   ...
   */ async getUnconfirmedTransactionsByTag(address, tag, firstIndex = null, lastIndex = null) {
        ...
        if (firstIndex && !Number.isInteger(firstIndex)) {
            throw new Error('firstIndex needs to be an int or null');
        }
        if (lastIndex && !Number.isInteger(lastIndex)) {
            throw new Error('lastIndex needs to be an int or null');
        }
        if (firstIndex && !lastIndex) {
            lastIndex = firstIndex + 10;
        }
        try {
            const response = await this.jupiterAPIService.getUnconfirmedBlockChainTransactions(address, tag, true, 1, true, firstIndex, lastIndex);
   ...
   */ async fetchConfirmedBlockChainTransactionsByTag(address, tag, firstIndex = null, lastIndex = null) {
        ...
        const transactionsResponse = await this.jupiterAPIService.getBlockChainTransactions(address, tag, true, 1, true, firstIndex, lastIndex);
   ...
   */ async fetchUnconfirmedMessages(accountProperties) {
        ...
        return await this.messageService.readMessagesFromMessageTransactionsAndDecryptOrPassThroughAndReturnMessageContainers(unconfirmedTransactionsResponse, accountProperties.crypto, accountProperties.passphrase);
    }
    /**
   ...
   */ fetchAllMessagesBySender(accountProperties, decipherWith = null) {
        ...
        logger.verbose('##');
        return this.getAllConfirmedAndUnconfirmedBlockChainTransactions(accountProperties.address).then((transactions)=>this.messageService.fetchAllMessagesBySender(transactions, accountProperties, decipherWith));
    }
    /**
   *
   ...
   */ async dereferenceListAndGetReadableTaggedMessageContainers(gravityAccountProperties, listTag, isMetisEncrypted = true) {
        ...
        try {
            ...
            latestReferenceList.forEach((transactionId)=>{
                messages.push(this.messageService.getReadableMessageContainerFromMessageTransactionIdAndDecryptOrPassThrough(transactionId, gravityAccountProperties.crypto, gravityAccountProperties.passphrase));
            });
   ...
   */ async fetchLatestReferenceList(gravityAccountProperties, listTag, isMetisEncrypted = true) {
        ...
        if (!listTag) {
            throw new mError.MetisError('listTag is invalid');
        }
        try {
...
module.exports.jupiterTransactionsService = new JupiterTransactionsService(jupiterAPIService, jupiterTransactionMessageService, transactionUtils, validator);
