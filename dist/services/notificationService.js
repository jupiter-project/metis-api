...
module.exports = {
   ...
   */ incrementBadgeCounter: (filter)=>{
        ...
        return Notifications.findOneAndUpdate(filter, {
            $inc: {
                'pnAccounts.$[].badgeCounter': 1
            }
        }, {
            new: true
        }).lean();
    ...
    findOneNotificationAndRemovePNToken: (filter, provider, token)=>{
        return Notifications.findOne(filter).lean().then(async (notification)=>{
            ...
            return Notifications.updateOne(filter, {
                $pull: {
                    pnAccounts: {
                        provider,
                        token
   ...
   */ findNotifications: (userAddresses, mutedUserAddresses = [])=>{
        if (!gu.isNonEmptyArray(userAddresses)) {
            throw new Error('addressList needs to be an array with values');
        }
        ...
        if (!Array.isArray(mutedUserAddresses)) {
            throw new Error('mutedUserAddresses needs to be an array');
        }
        mutedUserAddresses.forEach((mutedUserAddress)=>{
            if (!gu.isWellFormedJupiterAddress(mutedUserAddress)) {
                throw new mError.MetisErrorBadJupiterAddress(`mutedUserAddress: ${mutedUserAddress}`);
            }
        ...
        if (mutedUserAddresses.length > 0) {
            filter.mutedChannelAddressList = {
                $nin: mutedUserAddresses
            };
        }
   ...
   */ findNotificationItemsByToken: (token)=>{
    ...
    findMutedChannels: (userAddress)=>{
        ...
        return Notifications.find(filter).select('mutedChannelAddressList');
    }
};
