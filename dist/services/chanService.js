...
class ChanService {
   ...
   */ constructor(appAccountProperties, jupApi, jupiterTransactionsService, jupiterFundingService, jupiterAccountService, tableService, gravity, gravityService, transactionUtils, validator){
   ...
   */ async getChannelInvite(memberAccountProperties, channelAddress) {
        ...
        const transactions = await jupiterTransactionsService.fetchConfirmedAndUnconfirmedBlockChainTransactionsByTag(memberAccountProperties.address, tag);
        ...
        const messageContainer = await this.messageService.getReadableMessageContainerFromMessageTransactionId(transactionId, memberAccountProperties.passphrase);
   ...
   */ async getChannelInvitationContainersSentToAccount(sentToMemberAccountProperties) {
        ...
        try {
            ...
            const invitationContainers = await this.jupiterTransactionsService.getReadableTaggedMessageContainers(sentToMemberAccountProperties, recordTag, false);
            ...
            const invitationContainersNotInExistingChannels = invitationContainersFilteredByTo.filter((invitationContainer)=>!memberChannels.some((channel)=>channel.address === invitationContainer.message.channelRecord.address));
            ...
            const filteredInvitationContainers = invitationContainersNotInExistingChannels.filter((invitationContainer)=>!declinedInvitationContainers.some((declinedInvitationContainer)=>declinedInvitationContainer.transactionId === invitationContainer.transactionId));
   ...
   */ async createInvitation(channelAccountProperties, inviterAccountProperties, inviteeAddress, inviteePublicKey) {
        ...
        try {
            ...
            const sendTaggedAndEncipheredMetisMessageResponse = await this.jupiterTransactionsService.messageService.sendTaggedAndEncipheredMetisMessage(inviterAccountProperties.passphrase, inviteeAddress, JSON.stringify(inviteRecordJson), recordTag, feeType, inviteePublicKey);
   ...
   */ async acceptInvitation(memberAccountProperties, channelAddress) {
        ...
        try {
            ...
            const channelAccountPropertiesInvitedTo = await instantiateGravityAccountProperties(invitationRecord.channelRecord.passphrase, invitationRecord.channelRecord.password);
   ...
   */ async getChannelAccountPropertiesOrNullFromChannelRecordAssociatedToMember(memberAccountProperties, channelAddress) {
        ...
        try {
            ...
            const gravityAccountProperties = await instantiateGravityAccountProperties(channelAccountRecord.passphrase, channelAccountRecord.password);
   ...
   */ async fetchChannelAccountRecord(transactionId, crypto, passphrase) {
        ...
        const messageContainer = await this.jupiterTransactionsService.messageService.getReadableMessageContainerFromMessageTransactionIdAndDecrypt(transactionId, crypto, passphrase);
   ...
   */ async processNewMember(memberAccountProperties, channelAccountPropertiesInvitedTo, role = 'basic-member', createdBy = null) {
        ...
        const params = {
            channel: channelAccountPropertiesInvitedTo.address,
            password: channelAccountPropertiesInvitedTo.password,
            account: memberAccountProperties.address,
   ...
   */ async generateInviteRecordJson(channelName, inviteeAddress, inviteePublicKey, inviterAccountProperties, channelAccountProperties) {
        ...
        const date = Date.now();
        const channelRecordJson = await this.generateNewChannelRecordJson(channelName, channelAccountProperties, inviterAccountProperties.address);
        const inviteRecord = {
   ...
   */ async addChannelInfoToAccountIfDoesntExist(accountProperties, channelAccountProperties, createdBy = null) {
        ...
        logger.info('  +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++');
        const channelRecordPayload = await this.generateNewChannelRecordJson(channelAccountProperties.channelName, channelAccountProperties, createdBy || accountProperties.address);
        ...
        // }
        return this.jupiterTransactionsService.messageService.sendTaggedAndEncipheredMetisMessage(accountProperties.passphrase, accountProperties.address, encryptedChannelRecordPayload, recordTag, feeType, accountProperties.publicKey);
    }
    /**
   *
   * @returns {*}
   ...
   */ async addMemberInfoToChannelIfDoesntExist(memberProperties, channelProperties, role = 'basic-member') {
        ...
        try {
            ...
            const transactionResponse = await this.gravityService.addNewRecordToReferencedDataSet(newMemberPayload, channelProperties, listTag, recordTag);
            ...
            transactionsReport.push({
                name: 'channel-member-list',
                id: transactionIdForTheLatestTransactionsList
            });
   ...
   */ getChannelMembers(channelAccountProperties) {
        ...
        return jupiterTransactionsService.dereferenceListAndGetReadableTaggedMessageContainers(channelAccountProperties, listTag).then((messageContainers)=>uniqBy(messageContainers.map((messageContainer)=>messageContainer.message), 'memberAccountAddress'));
    }
    /**
   *
...
module.exports.chanService = new ChanService(metisGravityAccountProperties, jupiterAPIService, jupiterTransactionsService, jupiterFundingService, jupiterAccountService, tableService, gravity, gravityService, transactionUtils, validator);
