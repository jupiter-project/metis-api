...
class JupiterAccountService {
   ...
   */ async addUserRecordToUserAccount(userAccountProperties) {
        ...
        return this.jupiterTransactionsService.messageService.sendTaggedAndEncipheredMetisMessage(userAccountProperties.passphrase, userAccountProperties.address, encryptedUserRecord, tag, FeeManager.feeTypes.account_record, userAccountProperties.publicKey).then((response)=>{
   ...
   */ async generateId(accountProperties, metisUsersTableProperties) {
        ...
        return new Promise((resolve, reject)=>{
            this.jupiterAPIService.sendSimpleNonEncipheredMetisMessage(metisUsersTableProperties, accountProperties, 'Generating Id for record', fee, subtype, false).then((response)=>{
   ...
   */ async fetchAccountOrNull(address) {
        ...
        return await this.jupiterAPIService.getAccount(address).then((response)=>{
   ...
   */ async fetchAccountStatement(passphrase, password, statementId = '', accountType = '', params = {}) {
        ...
        try {
            ...
            for(let i = 0; i < attachedTablesProperties.length; i++){
                ...
                attachedTablesStatementsPromises.push(this.fetchAccountStatement(tableAccountProperties.passphrase, password, `table-${tableAccountProperties.name}`, 'table', {
   ...
   */ async addE2EPublicKeyToJupiterAccount(e2ePublicKey, gravityAccountProperties, userAddress = null, userAlias = '', accountType = 'UserAccount') {
        ...
        try {
            ...
            const latestE2EPublicKeysContainers = await this.jupiterTransactionsService.dereferenceListAndGetReadableTaggedMessageContainers(gravityAccountProperties, listTag);
            ...
            const userE2EPublicKeyResponse = await jupiterTransactionsService.messageService.sendTaggedAndEncipheredMetisMessage(gravityAccountProperties.passphrase, gravityAccountProperties.address, encryptedMessage, recordTag, FeeManager.feeTypes.account_record, gravityAccountProperties.publicKey);
            ...
            await jupiterTransactionsService.messageService.sendTaggedAndEncipheredMetisMessage(gravityAccountProperties.passphrase, gravityAccountProperties.address, encryptedLatestUserE2ETransactionIds, `${listTag}.${metisConfig.evm}`, FeeManager.feeTypes.account_record, gravityAccountProperties.publicKey);
        } catch (error) {
   ...
   */ getAliasesOrEmptyArray(address) {
        ...
        return this.jupiterAPIService.getAliases(address).then((getAliasesResponse)=>{
   ...
   */ isAliasAvailable(aliasName) {
        ...
        return this.jupiterAPIService.getAlias(aliasName).then((response)=>{
   ...
   */ async getStatement(accountProperties) {
        ...
        logger.verbose('##############################################################');
        return new Promise((resolve, reject)=>{
            this.jupiterAPIService.getBalance(accountProperties.address).then((response)=>{
   ...
   */ async getMemberAccountPropertiesFromPersistedUserRecordOrNull(memberPassphrase, memberPassword) {
        ...
        console.log('FF');
        const messageContainers = await jupiterTransactionsService.getReadableTaggedMessageContainers(memberAccountProperties, `${userConfig.userRecord}.${memberAccountProperties.address}`, true, null, null, (transaction)=>transaction.senderRS === memberAccountProperties.address);
...
module.exports.jupiterAccountService = new JupiterAccountService(jupiterAPIService, metisGravityAccountProperties, tableService, jupiterTransactionsService, gravityService, transactionUtils);
