...
class GravityService {
   ...
   */ async addNewRecordToReferencedDataSet(recordJson, gravityAccountProperties, listTag, recordTag, metisEncrypt = true, feeType = FeeManager.feeTypes.account_record) {
        ...
        try {
            ...
            const response = await this.messageService.sendTaggedAndEncipheredMetisMessage(gravityAccountProperties.passphrase, gravityAccountProperties.address, payload, recordTag, feeType, gravityAccountProperties.publicKey);
            ...
            return this.messageService.sendTaggedAndEncipheredMetisMessage(gravityAccountProperties.passphrase, gravityAccountProperties.address, updatedList, listTag, feeType, gravityAccountProperties.publicKey);
        } catch (error) {
            logger.error('****************************************************************');
   ...
   */ addItemToTransactionsReferenceList(gravityAccountProperties, newItem, listTag, feeType = FeeManager.feeTypes.account_record, isMetisEncrypted = true) {
        ...
        return this.getLatestListByTag(gravityAccountProperties, listTag, isMetisEncrypted).then(async (list)=>{
            ...
            // }
            return this.messageService.sendTaggedAndEncipheredMetisMessage(gravityAccountProperties.passphrase, gravityAccountProperties.address, encryptedList, listTag, feeType, gravityAccountProperties.publicKey);
        });
    }
    /**
   *
   ...
   */ removeItemFromTransactionsReferenceList(gravityAccountProperties, itemToRemove, listTag, feeType = FeeManager.feeTypes.account_record, isMetisEncrypted = true) {
        return this.getLatestListByTag(gravityAccountProperties, listTag, isMetisEncrypted).then(async (list)=>{
            ...
            return this.messageService.sendTaggedAndEncipheredMetisMessage(gravityAccountProperties.passphrase, gravityAccountProperties.address, encryptedNewList, listTag, feeType, gravityAccountProperties.publicKey);
        });
    }
    /**
   *
   ...
   */ async getLatestListByTag(accountProperties, tag, isMetisEncrypted = true) {
        ...
        return this.jupiterTransactionsService.getReadableTaggedMessageContainers(accountProperties, tag, isMetisEncrypted).then((messageContainers)=>{
