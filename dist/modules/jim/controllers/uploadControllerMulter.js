...
const uploadControllerMulter = async (req, res, next, app, jobs, websocket)=>{
    ...
    try {
        ...
        const fileCategory1 = fileUploadData.fileCategory;
        ...
        const WEBSOCKET_ROOM = fileCategory1 === fileCategoryTypes.publicProfile ? `upload-${userAccountProperties.address}` : `upload-${fileUploadData.attachToJupiterAddress}`;
        const fileUrl = fileCategory1 === fileCategoryTypes.publicProfile || fileCategory1 === fileCategoryTypes.channelProfile ? `/jim/v1/api/users/${fileUploadData.attachToJupiterAddress}/files/public-profile` : `/jim/v1/api/channels/${fileUploadData.attachToJupiterAddress}/files/${fileUuid}`;
        ...
        const params = fileCategory1 === fileCategoryTypes.publicProfile ? {} : {
            attachToJupiterAddress: fileUploadData.attachToJupiterAddress
        };
        const job = await uploadJob.create(fileUploadData.userAccountProperties, fileUploadData.fileName, fileUploadData.fileEncoding, fileUploadData.fileMimeType, fileUploadData.fileUuid, fileCategory1, params);
        job.save((error)=>{
