...
class UploadJob {
   ...
   */ initialize() {
        ...
        this.jobQueue.process(this.jobName, WORKERS, async (job, done)=>{
            ...
            try {
                const { fileCategory , userAccountProperties: _userAccountProperties , fileName , fileEncoding , fileMimeType , fileUuid  } = job.data;
                ...
                //
                    ...
                    if (!gu.isWellFormedJupiterAddress(job.data.attachToJupiterAddress)) {
                        return done(new mError.MetisErrorBadJupiterAddress('attachToJupiterAddress', job.data.attachToJupiterAddress));
                    }
                    const attachToAccountProperties = await this.channelService.getChannelAccountPropertiesOrNullFromChannelRecordAssociatedToMember(fromAccountProperties, attachToJupiterAddress);
                ...
                fs.readFile(fileBufferDataPath, async (error, bufferData)=>{
                    try {
                        const sendFileToBlockChainResponse = await this.storageService.sendFileToBlockchain(fileName, fileMimeType, fileUuid, bufferData, fromAccountProperties, toAccountProperties, fileCategory);
                        return done(null, sendFileToBlockChainResponse);
                    } catch (error1) {
                        logger.error('****************************************************************');
   ...
   */ create(ownerAccountProperties, fileName, fileEncoding, fileMimeType, fileUuid, fileCategory, params = {}) {
        ...
        if (!gu.isNonEmptyString(fileName)) throw new mError.MetisError('fileName is empty');
        if (!gu.isNonEmptyString(fileEncoding)) throw new mError.MetisError('fileEncoding is empty');
        if (!gu.isNonEmptyString(fileMimeType)) throw new mError.MetchatisError('fileMimeType is empty');
        if (!gu.isNonEmptyString(fileUuid)) throw new mError.MetisError('fileUuid is empty');
        // eslint-disable-next-line no-empty
        if (fileCategory === FILE_CATEGORY_TYPES.PUBLIC_PROFILE) {} else {
        ...
        const job = this.jobQueue.create(this.jobName, jobData).priority('high').removeOnComplete(false).save((error)=>{
            logger.verbose(`---- JOB.SAVE: ${this.jobName}.save()`);
            if (error) {
                console.log('\n');
                logger.error('************************* ERROR ***************************************');
