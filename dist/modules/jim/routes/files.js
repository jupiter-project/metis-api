...
function abort(request, response, busboy, statusCode = StatusCode.ServerErrorInternal, metisError) {
    ...
    if (!response.aborted) {
        ...
        response.status(statusCode).send({
            message: metisError.message,
            code: metisError.code
        });
    }
...
const uploadController = (req, res, next, app, jobs, websocket)=>{
    ...
    const bb = busboy({
        ...
        limits: {
            files: 1,
            fileSize: jimConfig.maxBytesSize
        }
    });
    try {
        ...
        bb.on('file', (formDataKey, file, info)=>{
            ...
            if (formDataKey !== 'file') {
                return abort(req, res, bb, StatusCode.ClientErrorNotAcceptable, new mError.MetisErrorBadRequestParams(`file key needs to be (file) not: ${formDataKey}`));
            // return res.status(StatusCode.ClientErrorNotAcceptable).send({
        ...
        bb.on('close', async ()=>{
            ...
            try {
                ...
                const WEBSOCKET_ROOM = fileCategory === fileCategoryTypes.publicProfile ? `upload-${userAccountProperties.address}` : `upload-${fileUploadData.attachToJupiterAddress}`;
                const fileUrl = fileCategory === fileCategoryTypes.publicProfile || fileCategory === fileCategoryTypes.channelProfile ? `/jim/v1/api/users/${fileUploadData.attachToJupiterAddress}/files/public-profile` : `/jim/v1/api/channels/${fileUploadData.attachToJupiterAddress}/files/${fileUuid}`;
                ...
                const job = await uploadJob.create(fileUploadData.userAccountProperties, fileUploadData.fileName, fileUploadData.fileEncoding, fileUploadData.fileMimeType, fileUploadData.fileUuid, fileCategory, params);
                job.save((error)=>{
    ...
    app.get('/jim/v1/api/users/:userAddress/files/public-profile', async (req, res)=>{
        ...
        try {
            ...
            if (messageContainerTag.includes(`.${metisConfig.evm}`)) {
                [fileUuid, transactionId, sharedKey] = messageContainers.attachment.message.split('.').slice(-4);
            } else {
