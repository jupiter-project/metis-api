 ...
 */ class StorageService {
   ...
   */ constructor(jupiterAPIService, jupiterTransactionsService, jupiterFundingService, jupiterAccountService, jimConfig, transactionUtils, chanService, fileCacheService){
   ...
   */ async fetchChannelFilesList(memberAccountProperties, channelAddress) {
        ...
        if (channelAccountProperties === null) {
            throw new mError.MetisErrorNoChannelAccountFound(`User ${memberAccountProperties.address} doesnt have ${channelAddress} channel account`);
        }
        ...
        const allFileRecordTransactions = await this.jupiterTransactionsService.fetchConfirmedAndUnconfirmedBlockChainTransactionsByTag(channelAccountProperties.address, fileTag);
        ...
        const filteredFileRecordTransactions = this.transactionUtils.filterTransactionsByTransactionIds(allFileRecordTransactions, blackList);
        ...
        const messageContainers = await this.jupiterTransactionsService.messageService.getReadableMessageContainersFromTransactions(filteredFileRecordTransactions, channelAccountProperties);
   ...
   */ async fetchFileInfo(ownerAccountProperties, fileUuid, tag = null) {
        ...
        try {
            ...
            if (this.fileCacheService.cachedFileExists(fileUuid)) {
                ...
                fileRecord = isString(tag) && tag.includes(`.${metisConfig.evm}`) ? ownerAccountProperties.crypto.decryptAndParseGCM(encryptedFileRecord) : ownerAccountProperties.crypto.decryptAndParse(encryptedFileRecord);
            } else {
                ...
                const encryptedFileRecord1 = isString(tag) && tag.includes(`.${metisConfig.evm}`) ? ownerAccountProperties.crypto.encryptJsonGCM(fileRecord) : ownerAccountProperties.crypto.encryptJson(fileRecord);
                this.fileCacheService.sendBufferDataToCache(fileUuid, bufferData);
                this.fileCacheService.sendFileRecordToCache(fileUuid, encryptedFileRecord1);
            }
   ...
   */ async fetchFileFromBlockChain(ownerAccountProperties, fileUuid) {
        ...
        try {
            ...
            const fileRecordMessageContainers = await this.jupiterTransactionsService.getReadableTaggedMessageContainers(ownerAccountProperties, fileRecordTag);
    ...
    async fetchFileInfoBySharedKey(transactionId, sharedKey, fileUuid, tag) {
        ...
        try {
            ...
            if (this.fileCacheService.cachedFileExists(fileUuid)) {
                ...
                bufferData = fetchFileFromBlockChainResponse.bufferData;
                fileRecord = fetchFileFromBlockChainResponse.fileRecord;
                const encryptedFileRecord1 = tag.includes(`.${metisConfig.evm}`) ? crypto.decryptAndParseGCM(fileRecord) : crypto.decryptAndParse(fileRecord);
                this.fileCacheService.sendBufferDataToCache(fileUuid, bufferData);
                this.fileCacheService.sendFileRecordToCache(fileUuid, encryptedFileRecord1);
            }
            if (!this.fileCacheService.bufferDataExists(fileUuid)) throw new mError.MetisError('Buffer Data does not exist!');
   ...
   */ async fetchFileFromBlockChainBySharedKey(transactionId, sharedKey, tag) {
        ...
        try {
            ...
            logger.info(' GET ALL THE CHUNKS');
            logger.info('-__-__-__-__-__-__-__-__-__-__-__-__-__-__-__--\n');
            const readableMessageContainer$ = chunkTransactionIds.map((chunkTransactionId)=>jupiterTransactionsService.getReadableMessageContainersBySharedKey(chunkTransactionId.transactionId, chunkTransactionId.sharedKey, tag));
     ...
     */ async sendFileToBlockchain(fileName, mimeType, fileUuid, bufferData, fromAccountProperties, toAccountProperties, fileCat) {
        console.log('\n\n\n');
        logger.info('======================================================================================');
        ...
        try {
            ...
            const sendMessageResponsePromises = chunks.map((chunk)=>{
                ...
                return this.jupiterTransactionsService.messageService.sendTaggedAndEncipheredMetisMessage(fromAccountProperties.passphrase, toAccountProperties.address, chunk, `${transactionTags.jimServerTags.binaryFileChunk}.${fileUuid}`, FeeManager.feeTypes.metisMessage, toAccountProperties.publicKey);
            });
            ...
            if (fileCat === FileCategory.PublicProfile || fileCat === FileCategory.ChannelProfile) {
                const allChunkTransactionsInfo = this.transactionUtils.extractTransactionInfoFromTransactionResponses(sendMessageResponses, [
                    'transactionId',
                    'nonce'
                ]);
                ...
                const allChunkTransactionsInfo1 = this.transactionUtils.extractTransactionInfoFromTransactionResponses(sendMessageResponses, [
                    'transactionId'
                ]);
                allChunkTransactionsData = allChunkTransactionsInfo1.map((info)=>info.transactionId);
            }
            console.log('\n');
            logger.info('+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++');
            ...
            const fileRecord = this.generateFileRecordJson(fileUuid, fileCat, fileName, mimeType, fileSizeInBytes, allChunkTransactionsData, fromAccountProperties.address, linkedFileRecords, fileUrl);
            const _fileRecord = fileCat === FileCategory.PublicProfile || fileCat === FileCategory.ChannelProfile ? JSON.stringify(fileRecord) : toAccountProperties.crypto.encryptJsonGCM(fileRecord);
            if (!this.fileCacheService.bufferDataExists(fileUuid)) {
                this.fileCacheService.sendBufferDataToCache(fileUuid, bufferData);
            }
            const sendMessageResponseFileRecord = await this.jupiterTransactionsService.messageService.sendTaggedAndEncipheredMetisMessage(toAccountProperties.passphrase, toAccountProperties.address, _fileRecord, `${transactionTags.jimServerTags.binaryFileRecord}.${fileUuid}`, FeeManager.feeTypes.metisMessage, toAccountProperties.publicKey);
            ...
            if (fileCat === FileCategory.PublicProfile || fileCat === FileCategory.ChannelProfile) {
                ...
                //   )
                return {
                    fileRecord: encryptedFileRecord,
                    sharedKey: xSharedKey
                };
            }
            const encryptedFileRecord1 = toAccountProperties.crypto.encryptJsonGCM(fileRecord);
            this.fileCacheService.sendFileRecordToCache(fileUuid, encryptedFileRecord1);
            return {
   ...
   */ generateFileRecordJson(fileUuid, fileCat, fileName, mimeType, sizeInBytes, chunkTransactionIds, createdByAddress, linkedFileRecords, fileUrl) {
...
module.exports.storageService = new StorageService(jupiterAPIService, jupiterTransactionsService, jupiterFundingService, jupiterAccountService, jimConfig, transactionUtils, chanService, localFileCacheService);
