        ...
        try {
            ...
            if (getAccountIdResponse.data.accountRS === null) {
                ...
                return res.status(StatusCode.SuccessOK).send({
                    message: 'Jupiter account created',
                    account: jupiterAccount
                });
            }
        } catch (error1) {
            logger.error('****************************************************************');
            ...
            console.log(error1);
            if (error1 instanceof mError.JupiterApiError) {
                return res.status(StatusCode.ServerErrorInternal).send({
            ...
            return res.status(StatusCode.ServerErrorInternal).send({
                message: `There was an error: ${error1.response}`,
                code: error1.code
            });
        }
    });
