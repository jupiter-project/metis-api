...
const createJob = (jobs, newAccountProperties, newAccountAlias, res, websocket, subscriberId, next = ()=>{})=>{
    ...
    const job = jobs.create('MetisJobRegisterJupiterAccount', {
   ...
   */ job.on('complete', function(jobData) {
    ...
    job.on('failed attempt', function(errorMessage, doneAttempts) {
        ...
        websocket.of(namespace).to(room).emit('signUpFailedAttempt', {
    ...
    job.on('failed', function(errorMessage) {
        ...
        logger.error('***********************************************************************************');
        logger.error(`errorMessage= ${errorMessage}`);
        websocket.of(namespace).to(room).emit('signUpFailed', {
            message: `${errorMessage}`
        });
    });
};
module.exports = (app, jobs, websocket)=>{
     ...
     */ v1SignUpPost: async (req, res, next)=>{
            ...
            const { alias , key , encryption_password  } = req.body;
        ...
        ipLoggerInfo: async (req, res)=>{
            ...
            gu.ipLoggerRepeatedIpAddress().then((data)=>res.send(data)).catch((error)=>res.status(StatusCode.ServerErrorInternal).send(error));
        },
        /**
     * V2/SIGNUP
     */ v2SignUpPost: async (req, res, next)=>{
            ...
            if (!password) return res.status(StatusCode.ClientErrorBadRequest).send({
                message: 'provide a password'
            });
            if (!alias) return res.status(StatusCode.ClientErrorBadRequest).send({
                message: 'provide an alias'
            });
            ...
            kue.Job.get(jobId, (err, job)=>{
                if (err) {
                    return res.status(StatusCode.ServerErrorInternal).send({
                        message: 'Not able to get the job status'
                    });
                }
                res.send({
                    status: job._state
