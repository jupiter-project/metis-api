   ...
   */ app.put('/v1/api/users/:userAddress/e2e-public-keys', async (req, res)=>{
        ...
        try {
            ...
            res.status(StatusCode.SuccessOK).send({
                message: 'Public key was successfully added'
            });
        } catch (error1) {
            if (error1 instanceof mError.MetisErrorPublicKeyExists) {
                return res.status(StatusCode.SuccessAlreadyReported).send({
                    message: 'Publickey was already entered'
                });
            }
            console.log('\n');
            logger.error('************************* ERROR ***************************************');
            ...
            res.status(StatusCode.ServerErrorInternal).send({
                message: 'Problem adding publicKey'
            });
        }
    });
    app.get('/v1/api/users/:userAddress/e2e-public-keys', async (req, res)=>{
        res.status(StatusCode.ClientErrorImATeapot).send({
