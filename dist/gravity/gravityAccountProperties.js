 ...
 */ class GravityAccountProperties extends JupiterAccountProperties {
   ...
   */ constructor(address, accountId, publicKey, passphrase, passwordHash, password, algorithm = encryptAlgorithm, email = '', firstName = '', lastName = '', applicationAccountProperties = null){
        if (!address) {
   ...
   */ static async Clone(gravityAccountProperties) {
        ...
        if (!gu.isWellFormedPassphrase(gravityAccountProperties.passphrase)) {
            throw new mError.MetisErrorBadJupiterPassphrase('gravityAccountProperties.passphrase');
        }
        if (!gu.isNonEmptyString(gravityAccountProperties.password)) {
            throw new mError.MetisError('gravityAccountProperties.password is invalid');
        }
        const instantiateGAP = require('./instantiateGravityAccountProperties').instantiateGravityAccountProperties;
        const newProperties = await instantiateGAP(gravityAccountProperties.passphrase, gravityAccountProperties.password);
        ...
        if (gravityAccountProperties.hasOwnProperty('applicationAccountProperties') && gravityAccountProperties.applicationAccountProperties) {
            const applicationProperties = new ApplicationAccountProperties(gravityAccountProperties.applicationAccountProperties.deadline, gravityAccountProperties.applicationAccountProperties.feeNQT, gravityAccountProperties.applicationAccountProperties.accountCreationFeeNQT, gravityAccountProperties.applicationAccountProperties.transferFeeNQT, gravityAccountProperties.applicationAccountProperties.minimumTableBalance, gravityAccountProperties.applicationAccountProperties.minimumAppBalance, gravityAccountProperties.applicationAccountProperties.moneyDecimals);
...
module.exports.metisGravityAccountProperties = new GravityAccountProperties(metisConf.appAddress, metisConf.appAccountId, metisConf.appPublicKey, metisConf.appPassphrase, metisConf.appPasswordHash, metisConf.appPassword, metisConf.appPasswordAlgorithm, metisConf.appEmail, metisConf.appName, '', metisApplicationAccountProperties);
