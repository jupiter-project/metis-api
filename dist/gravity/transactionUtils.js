...
class TransactionUtils {
   ...
   */ areWellFormedJupiterTransactionIds(transactionIds) {
        if (!Array.isArray(transactionIds)) {
            throw new Error('Not an array: transactionsIds');
        }
        if (!gu.isNonEmptyArray(transactionIds)) {
            return true;
   ...
   */ isValidEncryptedMessageTransaction(messageTransaction) {
        ...
        if (!(messageTransaction.hasOwnProperty('attachment') && messageTransaction.attachment.hasOwnProperty('encryptedMessage') && messageTransaction.attachment.encryptedMessage.hasOwnProperty('data'))) {
            return false;
        }
        return true;
    }
   ...
   */ isValidEncryptedMessageTransactionResponse(transactionResponse) {
        ...
        if (!(transactionResponse.hasOwnProperty('transactionJSON') && transactionResponse.transactionJSON.hasOwnProperty('attachment') && transactionResponse.transactionJSON.attachment.hasOwnProperty('encryptedMessage') && transactionResponse.transactionJSON.attachment.encryptedMessage.hasOwnProperty('data'))) {
            return false;
   ...
   */ extractTransactionId(transaction) {
        ...
        if (!this.isWellFormedJupiterTransactionId(transactionId)) {
            throw new Error('transactionId is not well formed');
        }
   ...
   */ extractTransactionIdFromTransactionResponse(transactionResponse) {
        ...
        if (!transactionResponse) {
            throw new Error('transactionResponse is empty');
        }
        // if(!transactionResponse.hasOwnProperty('data')){throw new Error(`transactionResponse is invalid. no data property`)}
        if (!transactionResponse.hasOwnProperty('transactionJSON')) {
            throw new Error('transactionResponse is invalid. no data.transactionJSON');
        }
   ...
   */ extractTransactionIds(transactions) {
        ...
        if (!this.areValidTransactions(transactions)) {
            throw new Error('transactions are invalid');
        }
   ...
   */ // extractTransactionIdsFromTransactions(transactions){
    ...
    filterMessageTransactionsBySenderOrRecipient(transactions, recipientOrSenderAddress) {
        if (!gu.isNonEmptyString(recipientOrSenderAddress)) {
            throw new Error('recipientOrSenderAddress is empty');
        }
        return this.filterMessageTransactionsByCallback(transactions, (transaction)=>transaction.recipientRS === recipientOrSenderAddress || transaction.senderRS === recipientOrSenderAddress);
    }
    /**
   *
   * @param transactions
   ...
   */ filterMessageTransactionsByRecipient(transactions, recipientAddress) {
        if (!gu.isNonEmptyString(recipientAddress)) {
            throw new Error('recipientRS is empty');
        }
        return this.filterMessageTransactionsByCallback(transactions, (transaction)=>transaction.recipientRS === recipientAddress);
    }
    /**
   ...
   */ filterEncryptedMessageTransactionsBySender(transactions, senderAddress) {
        if (!gu.isWellFormedJupiterAddress(senderAddress)) {
            throw new mError.MetisErrorBadJupiterAddress(`senderAddress: ${senderAddress}`);
        }
