 ...
 */ class ApplicationAccountProperties {
   ...
   * @param moneyDecimals
   */ constructor(deadline, feeNQT, accountCreationFeeNQT, transferFeeNQT, minimumTableBalance, minimumAppBalance, moneyDecimals){
        if (!deadline) {
            throw new Error('missing deadline');
        }
        if (!feeNQT) {
            throw new Error('missing feeNQT');
        }
        if (!accountCreationFeeNQT) {
            throw new Error('missing accountCreationFeeNQT');
        }
        if (!transferFeeNQT) {
            throw new Error('missing transferFeeNQT');
        }
        if (!minimumTableBalance) {
            throw new Error('missing minimumTableBalance');
        }
        if (!minimumAppBalance) {
            throw new Error('missing minimumAppBalance');
        }
        if (!moneyDecimals) {
            throw new Error('missing moneyDecimals');
        }
...
module.exports.metisApplicationAccountProperties = new ApplicationAccountProperties(process.env.JUPITER_DEADLINE, STANDARD_FEE, ACCOUNT_CREATION_FEE, jupTransferFee, MINIMUM_TABLE_BALANCE, MINIMUM_APP_BALANCE, process.env.JUPITER_MONEY_DECIMALS);
